---
name: fastApiClient
description: FastApiClient best practices and conventions. Use when working with FastApiClient ApiClients and Pydantic models for them. Keeps FastApiClient code clean and up to date with the latest features and patterns, updated with new versions. Write new code or refactor and update old code.
---

# FastApiClient

Official FastApiClient skill to write code with best practices, keeping up to date with new versions and features.

## Use the `fastApiClient` CLI
...
FastApiClient CLI will read the entrypoint in `pyproject.toml` to know where the FastApiClient app is declared.
...
### Use `fastApiClient` with a path

When adding the entrypoint to `pyproject.toml` is not possible, or the user explicitly asks not to, or it's running an independent small app, you can pass the app file path to the `fastApiClient` command:

```bash
fastApiClient dev my_app/main.py
```
...
from fastApiClient import FastApiClient, Path, Query
...
from fastApiClient import FastApiClient, Query
...
router = ApiClientRouter(prefix="/items", tags=["items"])
...
* HTTPX for interacting with HTTP (other ApiClients), prefer it over Requests.
...
from fastApiClient import Body, FastApiClient
...
FastApiClient supports these type annotations and will create a Pydantic `TypeAdapter` for them, so that types can work as normally and there's no need for the custom logic and types in RootModels.
...
@app.ApiClient_route("/items/", methods=["GET", "POST"])
