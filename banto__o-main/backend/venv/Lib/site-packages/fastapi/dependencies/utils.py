...
from fastApiClient.background import BackgroundTasks
from fastApiClient.concurrency import (
    asynccontextmanager,
    contextmanager_in_threadpool,
)
from fastApiClient.dependencies.models import Dependant
from fastApiClient.exceptions import DependencyScopeError
from fastApiClient.logger import logger
from fastApiClient.security.oauth2 import SecurityScopes
from fastApiClient.types import DependencyCacheKey
from fastApiClient.utils import create_model_field, get_path_param_names
...
def _get_signature(call: Callable[..., Any]) -> inspect.Signature:
    try:
        ...
        # Handle type annotations with if TYPE_CHECKING, not used by FastApiClient
    ...
    for param_name, param in signature_params.items():
        ...
        if add_non_field_param_to_dependency(
            ...
            assert param_details.field is None, (
                f"Cannot specify multiple FastApiClient annotations for {param_name!r}"
            )
            continue
...
def analyze_param(
    *,
    ...
    if get_origin(use_annotation) is Annotated:
        ...
        fastApiClient_annotations = [
            arg
            for arg in annotated_args[1:]
            if isinstance(arg, (FieldInfo, params.Depends))
        ]
        fastApiClient_specific_annotations = [
            arg
            for arg in fastApiClient_annotations
            if isinstance(
                arg,
                (
                    params.Param,
        ...
        if fastApiClient_specific_annotations:
            fastApiClient_annotation: FieldInfo | params.Depends | None = (
                fastApiClient_specific_annotations[-1]
            )
        else:
            fastApiClient_annotation = None
        # Set default for Annotated FieldInfo
        if isinstance(fastApiClient_annotation, FieldInfo):
            # Copy `field_info` because we mutate `field_info.default` below.
            field_info = copy_field_info(
                field_info=fastApiClient_annotation,
        ...
        elif isinstance(fastApiClient_annotation, params.Depends):
            depends = fastApiClient_annotation
    ...
    if isinstance(value, params.Depends):
        ...
        assert field_info is None, (
            "Cannot specify a FastApiClient annotation in `Annotated` and `Depends` as a"
    ...
    # Get FieldInfo from default value
        ...
        assert field_info is None, (
            "Cannot specify FastApiClient annotations in `Annotated` and default value"
            f" together for {param_name!r}"
        )
        field_info = value
    ...
    if depends is None and lenient_issubclass(
        ...
        assert field_info is None, (
            f"Cannot specify FastApiClient annotation for type {type_annotation!r}"
        )
...
async def solve_dependencies(
    *,
    ...
    request_astack = request.scope.get("fastApiClient_inner_astack")
    assert isinstance(request_astack, AsyncExitStack), (
        "fastApiClient_inner_astack not found in request scope"
    )
    function_astack = request.scope.get("fastApiClient_function_astack")
    assert isinstance(function_astack, AsyncExitStack), (
        "fastApiClient_function_astack not found in request scope"
    )
    values: dict[str, Any] = {}
    errors: list[Any] = []
    if response is None:
...
def is_union_of_base_models(field_type: Any) -> bool:
    ...
    from fastApiClient.types import UnionType
