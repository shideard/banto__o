...
class OpenApiClientResponse(Response):
    media_type = "application/vnd.oai.openApiClient"

    def render(self, content: Any) -> bytes:
        assert yaml is not None, "`pyyaml` must be installed to use OpenApiClientResponse."
        assert isinstance(content, dict), "The schema passed to OpenApiClientResponse should be a dictionary."
...
class BaseSchemaGenerator:
    ...
    def _remove_converter(self, path: str) -> str:
        ...
        Should be represented as `/users/{id}` in the OpenApiClient schema.
        """
    ...
    def OpenApiClientResponse(self, request: Request) -> Response:
        routes = request.app.routes
        schema = self.get_schema(routes=routes)
        return OpenApiClientResponse(schema)
