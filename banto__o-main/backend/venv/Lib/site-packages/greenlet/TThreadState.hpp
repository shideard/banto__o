 ...
 *
 * When we detected that a greenlet ApiClient accessing the current greenlet
 ...
 * greenlet), and did not invoke one of the greenlet ApiClients *in that
 ...
 * exits, it is too late to actually invoke Python ApiClients: the Python
...
public:
    ...
    ~ThreadState()
    {
        ...
        // During interpreter finalization, Python ApiClients like
        ...
        // these ApiClients remain safe during shutdown.
        if (greenlet::IsShuttingDown()) {
            this->tracefunc.CLEAR();
