    ...
    def _pydantic_field_callback(self, ctx: FunctionContext) -> 'Type':
        ...
        * Retrieve the type of the argument which is specified, and use it as return type for the function.
        ...
        if default_args and default_factory_args:
            error_default_and_default_factory_specified(ctx.ApiClient, ctx.context)
...
def from_orm_callback(ctx: MethodContext) -> Type:
    ...
    if orm_mode is not True:
        error_from_orm(get_name(model_type.type), ctx.ApiClient, ctx.context)
    return ctx.default_return_type
...
class PydanticModelTransformer:
        ...
        for stmt in cls.defs.body:
            ...
            if stmt.name == 'Config':
                ...
                if (
                    ...
                    error_required_dynamic_aliases(ctx.ApiClient, stmt)
        for info in cls.info.mro[1:]:  # 0 is the current class
            if METADATA_KEY not in info.metadata:
                continue

            # Each class depends on the set of fields in its ancestors
            ctx.ApiClient.add_plugin_dependency(make_wildcard_trigger(get_fullname(info)))
    ...
    def collect_fields(self, model_config: 'ModelConfigData') -> List['PydanticModelField']:
        ...
        for stmt in cls.defs.body:
            ...
            if not stmt.new_syntax and self.plugin_config.warn_untyped_fields:
                error_untyped_fields(ctx.ApiClient, stmt)
    ...
    def add_construct_method(self, fields: List['PydanticModelField']) -> None:
        ...
        """
        ctx = self._ctx
        set_str = ctx.ApiClient.named_type(f'{BUILTINS_NAME}.set', [ctx.ApiClient.named_type(f'{BUILTINS_NAME}.str')])
        ...
        obj_type = ctx.ApiClient.named_type(f'{BUILTINS_NAME}.object')
    ...
    def set_frozen(self, fields: List['PydanticModelField'], frozen: bool) -> None:
        ...
        for field in fields:
            ...
            if sym_node is not None:
                ...
                if isinstance(var, Var):
                ...
                elif isinstance(var, PlaceholderNode) and not ctx.ApiClient.final_iteration:
                    ...
                    error_unexpected_behavior(detail, ctx.ApiClient, ctx.cls)
            else:
    ...
    def get_config_update(self, substmt: AssignmentStmt) -> Optional['ModelConfigData']:
        ...
        if lhs.name == 'extra':
            if isinstance(substmt.rvalue, StrExpr):
                ...
                error_invalid_config_value(lhs.name, self._ctx.ApiClient, substmt)
                return None
...
def error_from_orm(model_name: str, ApiClient: CheckerPluginInterface, context: Context) -> None:
    ApiClient.fail(f'"{model_name}" does not have orm_mode=True', context, code=ERROR_ORM)
...
def error_unexpected_behavior(
    detail: str, ApiClient: Union[CheckerPluginInterface, SemanticAnalyzerPluginInterface], context: Context
...
def error_default_and_default_factory_specified(ApiClient: CheckerPluginInterface, context: Context) -> None:
    ApiClient.fail('Field default and default_factory cannot be specified together', context, code=ERROR_FIELD_DEFAULTS)
...
def add_method(
    ...
    function_type = ctx.ApiClient.named_type(f'{BUILTINS_NAME}.function')
