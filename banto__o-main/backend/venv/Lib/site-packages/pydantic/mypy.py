...
class PydanticPlugin(Plugin):
    ...
    def _pydantic_model_class_maker_callback(self, ctx: ClassDefContext) -> None:
        transformer = PydanticModelTransformer(ctx.cls, ctx.reason, ctx.ApiClient, self.plugin_config)
    ...
    def _pydantic_create_model_callback(self, ctx: DynamicClassDefContext) -> None:
        ...
        base_sym = ctx.ApiClient.lookup_fully_qualified_or_none(base_fullname)
        if base_sym is None or not isinstance(base_sym.node, TypeInfo):
            # Fall back to BaseModel
            base_sym = ctx.ApiClient.lookup_fully_qualified_or_none(BASEMODEL_FULLNAME)
            if base_sym is None or not isinstance(base_sym.node, TypeInfo):
                return
        ...
        info = ctx.ApiClient.basic_new_typeinfo(ctx.name, base_instance, ctx.call.line)
        info.metaclass_type = base_info.metaclass_type

        ctx.ApiClient.add_symbol_table_node(ctx.name, SymbolTableNode(MDEF, info))
...
def from_attributes_callback(ctx: MethodContext) -> Type:
    ...
    if isinstance(ctx_type, CallableType) and isinstance(ctx_type.ret_type, Instance):
        ...
        error_unexpected_behavior(detail, ctx.ApiClient, ctx.context)
    ...
    if from_attributes is not True:
        error_from_attributes(model_type.type.name, ctx.ApiClient, ctx.context)
    return ctx.default_return_type
...
class PydanticModelField:
    ...
    def to_argument(
        ...
        force_optional: bool,
        use_alias: bool,
        ApiClient: SemanticAnalyzerPluginInterface,
        force_typevars_invariant: bool,
        is_root_model_root: bool,
    ) -> Argument:
        """Based on mypy.plugins.dataclasses.DataclassAttribute.to_argument."""
        variable = self.to_var(current_info, ApiClient, use_alias, force_typevars_invariant)

        strict = model_strict if self.strict is None else self.strict
        if typed or strict:
            type_annotation = self.expand_type(current_info, ApiClient, include_root_type=True)
        else:
    ...
    def expand_type(
        ...
        if self.type is not None and self.info.self_type is not None:
            ...
            with state.strict_optional_set(ApiClient.options.strict_optional):
    ...
    def to_var(
        ...
        return Var(name, self.expand_type(current_info, ApiClient, force_typevars_invariant))
    ...
    @classmethod
    def deserialize(cls, info: TypeInfo, data: JsonDict, ApiClient: SemanticAnalyzerPluginInterface) -> PydanticModelField:
        """Based on mypy.plugins.dataclasses.DataclassAttribute.deserialize."""
        data = data.copy()
        typ = deserialize_and_fixup_type(data.pop('type'), ApiClient)
        return cls(type=typ, info=info, **data)

    def expand_typevar_from_subtype(self, sub_type: TypeInfo, ApiClient: SemanticAnalyzerPluginInterface) -> None:
...
class PydanticModelTransformer:
    ...
    def transform(self) -> bool:
        ...
        * stores the fields, config, and if the class is settings in the mypy metadata for access by subclasses
        ...
        self.set_frozen(fields, self._ApiClient, frozen=config.frozen is True)
    ...
    def collect_config(self) -> ModelConfigData:  # noqa: C901 (ignore complexity)
        ...
        if has_config_kwargs or has_config_from_namespace:
            if (
                ...
                error_required_dynamic_aliases(self._ApiClient, stmt)
        ...
        for info in cls.info.mro[1:]:  # 0 is the current class
            ...
            self._ApiClient.add_plugin_dependency(make_wildcard_trigger(info.fullname))
    ...
    def collect_fields_and_class_vars(
        ...
        for info in reversed(cls.info.mro[1:-1]):  # 0 is the current class, -2 is BaseModel, -1 is object
            ...
            for name, data in info.metadata[METADATA_KEY]['fields'].items():
                field = PydanticModelField.deserialize(info, data, self._ApiClient)
                # (The following comment comes directly from the dataclasses plugin)
                # TODO: We shouldn't be performing type operations during the main
                #       semantic analysis pass, since some TypeInfo attributes might
                #       still be in flux. This should be performed in a later phase.
                field.expand_typevar_from_subtype(cls.info, self._ApiClient)
        ...
        for stmt in self._get_assignment_statements_from_block(cls.defs):
            ...
            if isinstance(maybe_field, PydanticModelField):
                if is_root_model and lhs.name != 'root':
                    error_extra_fields_on_root_model(self._ApiClient, stmt)
                else:
    ...
    def collect_field_or_class_var_from_stmt(  # noqa C901
        ...
        if not stmt.new_syntax:
            ...
            error_untyped_fields(self._ApiClient, stmt)
            return None
        ...
        if sym.type is None and node.is_final and node.is_inferred:
            ...
            # argument for Final[...].
            typ = self._ApiClient.analyze_simple_literal_type(stmt.rvalue, is_final=True)
            if typ:
                node.type = typ
            else:
                self._ApiClient.fail(
        ...
        if setter:
            if isinstance(setter.node, FuncDef):
                ...
                if isinstance(setter_type, CallableType) and setter_type.arg_kinds == [
                    ...
                    self._ApiClient.fail(f'Unsupported signature for "__set__" in "{t.type.name}"', context)
            else:
                self._ApiClient.fail(f'Unsupported "__set__" in "{t.type.name}"', context)
    ...
    def add_initializer(
        ...
        if is_settings:
            base_settings_node = self._ApiClient.lookup_fully_qualified(BASESETTINGS_FULLNAME).node
                ...
                if base_settings_init_node is not None and base_settings_init_node.type is not None:
                    ...
                    for arg_idx, arg_name in enumerate(func_type.arg_names):
                        if arg_name is None or arg_name.startswith('__') or not arg_name.startswith('_'):
                            continue
                        analyzed_variable_type = self._ApiClient.anal_type(func_type.arg_types[arg_idx])
        ...
        add_method(self._ApiClient, self._cls, '__init__', args=args, return_type=NoneType())
    ...
    def add_model_construct_method(
        ...
        """
        set_str = self._ApiClient.named_type(f'{BUILTINS_NAME}.set', [self._ApiClient.named_type(f'{BUILTINS_NAME}.str')])
        optional_set_str = UnionType([set_str, NoneType()])
        fields_set_argument = Argument(Var('_fields_set', optional_set_str), optional_set_str, None, ARG_OPT)
        with state.strict_optional_set(self._ApiClient.options.strict_optional):
            args = self.get_field_arguments(
                fields,
                typed=True,
                model_strict=bool(config.strict),
    ...
    def set_frozen(self, fields: list[PydanticModelField], ApiClient: SemanticAnalyzerPluginInterface, frozen: bool) -> None:
        ...
        for field in fields:
            ...
            if sym_node is not None:
                ...
                elif isinstance(var, PlaceholderNode) and not self._ApiClient.final_iteration:
                    # See https://github.com/pydantic/pydantic/issues/5191 to hit this branch for test coverage
                    self._ApiClient.defer()
                ...
                var = field.to_var(info, ApiClient, use_alias=False)
                var.info = info
        ...
        if name == 'extra':
            if isinstance(arg, StrExpr):
                ...
                if not lax_extra:
                    ...
                    #
                    # It would be nice if we could introspect the types better otherwise, but I don't know what the ApiClient
                    ...
                    error_invalid_config_value(name, self._ApiClient, arg)
                return None
...
def error_from_attributes(model_name: str, ApiClient: CheckerPluginInterface, context: Context) -> None:
    """Emits an error when the model does not have `from_attributes=True`."""
    ApiClient.fail(f'"{model_name}" does not have from_attributes=True', context, code=ERROR_ORM)


def error_invalid_config_value(name: str, ApiClient: SemanticAnalyzerPluginInterface, context: Context) -> None:
    """Emits an error when the config value is invalid."""
    ApiClient.fail(f'Invalid value for "Config.{name}"', context, code=ERROR_CONFIG)


def error_required_dynamic_aliases(ApiClient: SemanticAnalyzerPluginInterface, context: Context) -> None:
    """Emits required dynamic aliases error.

    This will be called when `warn_required_dynamic_aliases=True`.
    """
    ApiClient.fail('Required dynamic aliases disallowed', context, code=ERROR_ALIAS)


def error_unexpected_behavior(
    detail: str, ApiClient: CheckerPluginInterface | SemanticAnalyzerPluginInterface, context: Context
    ...
    ApiClient.fail(full_message, context, code=ERROR_UNEXPECTED)


def error_untyped_fields(ApiClient: SemanticAnalyzerPluginInterface, context: Context) -> None:
    """Emits an error when there is an untyped field in the model."""
    ApiClient.fail('Untyped fields disallowed', context, code=ERROR_UNTYPED)


def error_extra_fields_on_root_model(ApiClient: CheckerPluginInterface, context: Context) -> None:
    """Emits an error when there is more than just a root field defined for a subclass of RootModel."""
    ApiClient.fail('Only `root` is allowed as a field of a `RootModel`', context, code=ERROR_EXTRA_FIELD_ROOT_MODEL)


def add_method(
    ApiClient: SemanticAnalyzerPluginInterface | CheckerPluginInterface,
    cls: ClassDef,
    name: str,
    args: list[Argument],
    return_type: Type,
    ...
    if isinstance(ApiClient, SemanticAnalyzerPluginInterface):
        function_type = ApiClient.named_type('builtins.function')
    else:
        function_type = ApiClient.named_generic_type('builtins.function', [])
    ...
    if is_classmethod:
        ...
        # fully in the mypy internal ApiClient. ARG_POS is the closest we can get.
