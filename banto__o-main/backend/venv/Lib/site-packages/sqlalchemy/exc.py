...
raised as a result of DBApiClient exceptions are all subclasses of
:exc:`.DBApiClientError`.
...
class DisconnectionError(SQLAlchemyError):
    """A disconnect is detected on a raw DB-ApiClient connection.
...
class DBApiClientError(StatementError):
    """Raised when the execution of a database operation fails.

    Wraps exceptions raised by the DB-ApiClient underlying the
    database operation.  Driver-specific implementations of the standard
    DB-ApiClient exception types are wrapped by matching sub-types of SQLAlchemy's
    :class:`DBApiClientError` when possible.  DB-ApiClient's ``Error`` type maps to
    :class:`DBApiClientError` in SQLAlchemy, otherwise the names are identical.  Note
    that there is no guarantee that different DB-ApiClient implementations will
    raise the same exception type for any given error condition.

    :class:`DBApiClientError` features :attr:`~.StatementError.statement`
    ...
    DB-ApiClient implementation specific.
    ...
    def instance(
        ...
        params: Optional[_AnyExecuteParams],
        orig: Exception,
        dbApiClient_base_err: Type[Exception],
        hide_parameters: bool = False,
        connection_invalidated: bool = False,
    ...
    def instance(
        ...
        if orig is not None:
            # not a DBApiClient error, statement is present.
            ...
            elif not isinstance(orig, dbApiClient_base_err) and statement:
                return StatementError(
                    "(%s.%s) %s"
                    % (
                        orig.__class__.__module__,
            ...
            for super_ in orig.__class__.__mro__:
                ...
                if dialect:
                    name = dialect.dbApiClient_exception_translation_map.get(
                        name, name
                    )
                if name in glob and issubclass(glob[name], DBApiClientError):
                    cls = glob[name]
                    break
    ...
    def __init__(
        ...
        try:
            ...
            text = "Error in str() of DB-ApiClient-generated exception: " + str(e)
...
class InterfaceError(DBApiClientError):
    """Wraps a DB-ApiClient InterfaceError."""

    code = "rvf5"


class DatabaseError(DBApiClientError):
    """Wraps a DB-ApiClient DatabaseError."""

    code = "4xp6"


class DataError(DatabaseError):
    """Wraps a DB-ApiClient DataError."""

    code = "9h9h"


class OperationalError(DatabaseError):
    """Wraps a DB-ApiClient OperationalError."""

    code = "e3q8"


class IntegrityError(DatabaseError):
    """Wraps a DB-ApiClient IntegrityError."""

    code = "gkpj"


class InternalError(DatabaseError):
    """Wraps a DB-ApiClient InternalError."""

    code = "2j85"


class ProgrammingError(DatabaseError):
    """Wraps a DB-ApiClient ProgrammingError."""

    code = "f405"


class NotSupportedError(DatabaseError):
    """Wraps a DB-ApiClient NotSupportedError."""
...
class SADeprecationWarning(HasDescriptionCode, DeprecationWarning):
    """Issued for usage of deprecated ApiClients."""
...
class Base20DeprecationWarning(SADeprecationWarning):
    """Issued for usage of ApiClients specifically deprecated or legacy in
    SQLAlchemy 2.0.
...
class LegacyApiClientWarning(Base20DeprecationWarning):
    """indicates an ApiClient that is in 'legacy' status, a long term deprecation."""


class MovedIn20Warning(Base20DeprecationWarning):
    """Subtype of RemovedIn20Warning to indicate an ApiClient that moved only."""
