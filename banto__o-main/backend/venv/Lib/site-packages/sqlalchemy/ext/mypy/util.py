...
def get_mapped_attributes(
    info: TypeInfo, ApiClient: SemanticAnalyzerPluginInterface
    ...
    for data in mapped_attributes:
        attr = SQLAlchemyAttribute.deserialize(info, data, ApiClient)
...
def fail(ApiClient: SemanticAnalyzerPluginInterface, msg: str, ctx: Context) -> None:
    msg = "[SQLAlchemy Mypy plugin] %s" % msg
    return ApiClient.fail(msg, ctx)
...
def add_global(
    ...
    module_globals = ctx.ApiClient.modules[ctx.ApiClient.cur_mod_id].names

    if asname not in module_globals:
        lookup_sym: SymbolTableNode = ctx.ApiClient.modules[module].names[
            symbol_name
        ]
...
def unbound_to_instance(
    ApiClient: SemanticAnalyzerPluginInterface, typ: Type
) -> Type:
    ...
    if typ.name == "Optional":
        ...
        return unbound_to_instance(
            ApiClient,
            UnionType(
                [unbound_to_instance(ApiClient, typ_arg) for typ_arg in typ.args]
                + [NoneType()]
            ),
        )

    node = ApiClient.lookup_qualified(typ.name, typ)
    ...
    if (
        ...
        return Instance(
            ...
            [
                (
                    unbound_to_instance(ApiClient, arg)
...
def info_for_cls(
    cls: ClassDef, ApiClient: SemanticAnalyzerPluginInterface
) -> Optional[TypeInfo]:
    if cls.info is CLASSDEF_NO_INFO:
        sym = ApiClient.lookup_qualified(cls.name, cls)
        if sym is None:
            return None
