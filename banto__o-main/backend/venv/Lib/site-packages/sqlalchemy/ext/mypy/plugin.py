...
class SQLAlchemyPlugin(Plugin):
    ...
    def get_additional_deps(
        ...
        return [
            ...
            (10, "sqlalchemy.orm.decl_ApiClient", -1),
        ]
...
def _dynamic_class_hook(ctx: DynamicClassDefContext) -> None:
    ...
    cls.fullname = ctx.ApiClient.qualified_name(ctx.name)

    info = TypeInfo(SymbolTable(), cls, ctx.ApiClient.cur_mod_id)
    cls.info = info
    _set_declarative_metaclass(ctx.ApiClient, cls)
    ...
    if cls_arg is not None and isinstance(cls_arg.node, TypeInfo):
        ...
        decl_class.scan_declarative_assignments_and_apply_types(
            cls_arg.node.defn, ctx.ApiClient, is_mixin_scan=True
        )
        info.bases = [Instance(cls_arg.node, [])]
    else:
        obj = ctx.ApiClient.named_type(names.NAMED_TYPE_BUILTINS_OBJECT)
    ...
    try:
        ...
        util.fail(
            ctx.ApiClient, "Not able to calculate MRO for declarative base", ctx.call
        )
        obj = ctx.ApiClient.named_type(names.NAMED_TYPE_BUILTINS_OBJECT)
        info.bases = [obj]
        info.fallback_to_any = True

    ctx.ApiClient.add_symbol_table_node(ctx.name, SymbolTableNode(GDEF, info))
...
def _fill_in_decorators(ctx: ClassDefContext) -> None:
    for decorator in ctx.cls.decorators:
        ...
        if isinstance(target.expr, NameExpr):
            sym = ctx.ApiClient.lookup_qualified(
...
def _cls_decorator_hook(ctx: ClassDefContext) -> None:
    ...
    decl_class.scan_declarative_assignments_and_apply_types(ctx.cls, ctx.ApiClient)
...
def _base_cls_decorator_hook(ctx: ClassDefContext) -> None:
    ...
    decl_class.scan_declarative_assignments_and_apply_types(
        cls, ctx.ApiClient, is_mixin_scan=True
    )
...
def _declarative_mixin_hook(ctx: ClassDefContext) -> None:
    ...
    util.set_is_base(ctx.cls.info)
    decl_class.scan_declarative_assignments_and_apply_types(
        ctx.cls, ctx.ApiClient, is_mixin_scan=True
    )
...
def _set_declarative_metaclass(
    ApiClient: SemanticAnalyzerPluginInterface, target_cls: ClassDef
) -> None:
    info = target_cls.info
    sym = ApiClient.lookup_fully_qualified_or_none(
        "sqlalchemy.orm.decl_ApiClient.DeclarativeMeta"
    )
