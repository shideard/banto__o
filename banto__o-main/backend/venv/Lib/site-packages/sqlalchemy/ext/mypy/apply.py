...
def apply_mypy_mapped_attr(
    ...
    for stmt in cls.defs.body:
        ...
        util.fail(ApiClient, f"Can't find mapped attribute {name}", cls)
        return None
    ...
    apply_type_to_mapped_statement(
        ApiClient, stmt, stmt.lvalues[0], left_hand_explicit_type, None
    )
    ...
    for stmt in cls.defs.body:
        ...
        if (
            ...
            if (
                ...
                left_node.type = ApiClient.named_type(
...
def add_additional_orm_attributes(
    ...
    if "__init__" not in info.names and not is_base:
        ...
        for base in info.mro[1:-1]:
            ...
            base_cls_attributes = util.get_mapped_attributes(base, ApiClient)
        ...
        add_method_to_class(ApiClient, cls, "__init__", arguments, NoneTyp())

    if "__table__" not in info.names and util.get_has_table(info):
        _apply_placeholder_attr_to_class(
            ApiClient, cls, "sqlalchemy.sql.schema.Table", "__table__"
        )
    if not is_base:
        _apply_placeholder_attr_to_class(
            ApiClient, cls, "sqlalchemy.orm.mapper.Mapper", "__mapper__"
        )
...
def _apply_placeholder_attr_to_class(
    ...
    sym = ApiClient.lookup_fully_qualified_or_none(qualified_name)
    if sym:
