...
if TYPE_CHECKING:
    from ..engine.interfaces import DBApiClientConnection
    from ..engine.interfaces import DBApiClientCursor
...
class PoolResetState:
    """describes the state of a DBApiClient connection as it is being passed to
    ...
    """Indicates if the transaction on the DBApiClient connection was already
...
class _ConnDialect:
    ...
    which provides DBApiClient connection methods.
...
class _CreatorFnType(Protocol):
    def __call__(self) -> DBApiClientConnection: ...


class _CreatorWRecFnType(Protocol):
    def __call__(self, rec: ConnectionPoolEntry) -> DBApiClientConnection: ...
...
class Pool(log.Identified, event.EventTarget):
    ...
    def __init__(
        ...
        :param creator: a callable function that returns a DB-ApiClient
         ...
         * ``None`` - don't do anything on the connection.
           This setting may be appropriate if the database / DBApiClient
        ...
        :param dialect: a :class:`.Dialect` that will handle the job
         of calling rollback(), close(), or commit() on DBApiClient connections.
    ...
    def _close_connection(
        self, connection: DBApiClientConnection, *, terminate: bool = False
    ) -> None:
        self.logger.debug(
            "%s connection %r",
    ...
    def connect(self) -> PoolProxiedConnection:
        """Return a DBApiClient connection from the pool.
    ...
    def _return_conn(self, record: ConnectionPoolEntry) -> None:
        ...
        This method is called when an instrumented DBApiClient connection
...
class ManagesConnection:
    ...
    These two objects are typically exposed in the public facing ApiClient
    ...
    dbApiClient_connection: Optional[DBApiClientConnection]
    """A reference to the actual DBApiClient connection being tracked.

    This is a :pep:`249`-compliant object that for traditional sync-style
    dialects is provided by the third-party
    DBApiClient implementation in use.  For asyncio dialects, the implementation
    ...
    SQLAlchemy's interface for the DBApiClient connection is based on the
    :class:`.DBApiClientConnection` protocol object

    .. seealso::

        :attr:`.ManagesConnection.driver_connection`

        :ref:`faq_dbApiClient_connection`

    """

    driver_connection: Optional[Any]
    """The "driver level" connection object as used by the Python
    DBApiClient or database driver.

    For traditional :pep:`249` DBApiClient implementations, this object will
    be the same object as that of
    :attr:`.ManagesConnection.dbApiClient_connection`.   For an asyncio database
    ...
    .. seealso::
        ...
        :attr:`.ManagesConnection.dbApiClient_connection`
    ...
    def info(self) -> _InfoType:
        ...
        of the DBApiClient connection itself, including across pool checkins
...
class ConnectionPoolEntry(ManagesConnection):
    ...
    ApiClient code when it is delivered to connection pool event hooks, such as
    ...
    def close(self) -> None:
        """Close the DBApiClient connection managed by this connection pool entry."""
        raise NotImplementedError()
...
class _ConnectionRecord(ConnectionPoolEntry):
    ...
    to provide context management to a DBApiClient connection maintained by
    ...
    class for public ApiClient details.
    ...
    finalize_callback: Deque[Callable[[DBApiClientConnection], None]]
    fresh: bool
    ...
    def __init__(self, pool: Pool, connect: bool = True):
        ...
        self.fairy_ref = None
        self.starttime = 0
        self.dbApiClient_connection = None
    ...
    def driver_connection(self) -> Optional[Any]:  # type: ignore[override]  # mypy#4125  # noqa: E501
        if self.dbApiClient_connection is None:
            return None
        else:
            return self.__pool._dialect.get_driver_connection(
                self.dbApiClient_connection
            )
    ...
    def connection(self) -> Optional[DBApiClientConnection]:
        return self.dbApiClient_connection
    ...
    def checkout(cls, pool: Pool) -> _ConnectionFairy:
        ...
        try:
            dbApiClient_connection = rec.get_connection()
        ...
        fairy = _ConnectionFairy(pool, dbApiClient_connection, rec, echo)
        ...
        if echo:
            pool.logger.debug(
                "Connection %r checked out from pool", dbApiClient_connection
            )
        return fairy
    ...
    def checkin(self, _fairy_was_created: bool = True) -> None:
        if self.fairy_ref is None and _fairy_was_created:
            ...
            return
        self.fairy_ref = None
        connection = self.dbApiClient_connection
        pool = self.__pool
    ...
    def close(self) -> None:
        if self.dbApiClient_connection is not None:
            self.__close()
    ...
    def invalidate(
        ...
        if soft:
            self.__pool.dispatch.soft_invalidate(
                self.dbApiClient_connection, self, e
            )
        else:
            self.__pool.dispatch.invalidate(self.dbApiClient_connection, self, e)
        if e is not None:
    ...
    def get_connection(self) -> DBApiClientConnection:
        recycle = False
        ...
        assert self.dbApiClient_connection is not None
        return self.dbApiClient_connection

    def _is_hard_or_soft_invalidated(self) -> bool:
        return (
            self.dbApiClient_connection is None
    ...
    def __close(self, *, terminate: bool = False) -> None:
        ...
        if self.__pool.dispatch.close:
            self.__pool.dispatch.close(self.dbApiClient_connection, self)
        assert self.dbApiClient_connection is not None
        self.__pool._close_connection(
            self.dbApiClient_connection, terminate=terminate
        )
        self.dbApiClient_connection = None
    ...
    def __connect(self) -> None:
        ...
        try:
            ...
            self.dbApiClient_connection = connection = pool._invoke_creator(self)
            ...
            if pool.dispatch.first_connect:
                pool.dispatch.first_connect.for_modify(
                    pool.dispatch
                ).exec_once_unless_exception(self.dbApiClient_connection, self)

            # init of the dialect now takes place within the connect
            # event, so ensure a mutex is used on the first run
            pool.dispatch.connect.for_modify(
                pool.dispatch
            )._exec_w_sync_on_first_run(self.dbApiClient_connection, self)
...
def _finalize_fairy(
    ...
    if is_gc_cleanup:
        ...
        if connection_record.fairy_ref is not ref:
            return
        assert dbApiClient_connection is None
        dbApiClient_connection = connection_record.dbApiClient_connection
    ...
    if dbApiClient_connection is not None:
        if connection_record and echo:
            pool.logger.debug(
                "Connection %r being returned to pool", dbApiClient_connection
            )
        ...
        try:
            ...
            assert fairy.dbApiClient_connection is dbApiClient_connection
            ...
            if detach:
                ...
                if can_close_or_terminate_connection:
                    if pool.dispatch.close_detached:
                        pool.dispatch.close_detached(dbApiClient_connection)
            ...
            if detach and is_gc_cleanup and dont_restore_gced:
                message = (
                    ...
                    f"non-checked-in connection {dbApiClient_connection!r}, "
                    f"""which will be {
    ...
    if fairy is not None:
        fairy.dbApiClient_connection = None  # type: ignore
        fairy._connection_record = None
    del dbApiClient_connection
...
class PoolProxiedConnection(ManagesConnection):
    """A connection-like adapter for a :pep:`249` DBApiClient connection, which
    ...
    def is_valid(self) -> bool:
        ...
        to an active DBApiClient connection."""
...
class _AdhocProxiedConnection(PoolProxiedConnection):
    ...
    the DBApiClient connection is not actually proxied.
    ...
    __slots__ = ("dbApiClient_connection", "_connection_record", "_is_valid")

    dbApiClient_connection: DBApiClientConnection
    ...
    def __init__(
        ...
        self.dbApiClient_connection = dbApiClient_connection
    ...
    @property
    def connection(self) -> DBApiClientConnection:
        return self.dbApiClient_connection
    ...
    def cursor(self, *args: Any, **kwargs: Any) -> DBApiClientCursor:
        return self.dbApiClient_connection.cursor(*args, **kwargs)

    def __getattr__(self, key: Any) -> Any:
        return getattr(self.dbApiClient_connection, key)


class _ConnectionFairy(PoolProxiedConnection):
    """Proxies a DBApiClient connection and provides return-on-dereference
    support.

    This is an internal object used by the :class:`_pool.Pool` implementation
    to provide context management to a DBApiClient connection delivered by
    that :class:`_pool.Pool`.   The public facing interface for this class
    ...
    only for the length of a specific DBApiClient connection being checked out from
    ...
    def _checkout(
        ...
        assert (
            fairy.dbApiClient_connection is not None
        ...
        while attempts > 0:
            ...
            try:
                ...
                pool.dispatch.checkout(
                    fairy.dbApiClient_connection, fairy._connection_record, fairy
                )
                return fairy
    ...
    @property
    def is_valid(self) -> bool:
        return self.dbApiClient_connection is not None
    ...
    def invalidate(
        ...
        if not soft:
            ...
            # the connection
            self.dbApiClient_connection = None  # type: ignore
    ...
    def detach(self) -> None:
        if self._connection_record is not None:
            ...
            rec.fairy_ref = None
            rec.dbApiClient_connection = None
            ...
            if self._pool.dispatch.detach:
                self._pool.dispatch.detach(self.dbApiClient_connection, rec)
