...
class ConnectionKiller:
    def __init__(self):
        ...
        self.dbApiClient_connections = set()
    ...
    def _add_conn(self, dbApiClient_con, con_record, con_proxy):
        self.dbApiClient_connections.add(dbApiClient_con)
        self.proxy_refs[con_proxy] = True

    def _remove_conn(self, dbApiClient_conn, *arg):
        self.dbApiClient_connections.discard(dbApiClient_conn)
    ...
    def checkin_all(self):
        ...
        for rec in list(self.proxy_refs):
            if rec is not None and rec.is_valid:
                self.dbApiClient_connections.discard(rec.dbApiClient_connection)
        ...
        for con in self.dbApiClient_connections:
            self._safe(con.rollback)
        self.dbApiClient_connections.clear()
...
class ReconnectFixture:
    def __init__(self, dbApiClient):
        self.dbApiClient = dbApiClient
        self.connections = []
        self.is_stopped = False

    def __getattr__(self, key):
        return getattr(self.dbApiClient, key)

    def connect(self, *args, **kwargs):
        conn = self.dbApiClient.connect(*args, **kwargs)
        if self.is_stopped:
...
def reconnecting_engine(url=None, options=None):
    ...
    dbApiClient = config.db.dialect.dbApiClient
    if not options:
        options = {}
    options["module"] = ReconnectFixture(dbApiClient)
    engine = testing_engine(url, options)
    _dispose = engine.dispose

    def dispose():
        engine.dialect.dbApiClient.shutdown()
        engine.dialect.dbApiClient.is_stopped = False
        _dispose()

    engine.test_shutdown = engine.dialect.dbApiClient.shutdown
    engine.test_restart = engine.dialect.dbApiClient.restart
...
class DBApiClientProxyCursor:
    """Proxy a DBApiClient cursor.

    Tests can provide subclasses of this to intercept
    DBApiClient-level cursor operations.
...
class DBApiClientProxyConnection:
    """Proxy a DBApiClient connection.

    Tests can provide subclasses of this to intercept
    DBApiClient-level connection operations.
