...
class PingTest(fixtures.TestBase):
    ...
    def test_do_ping(self):
        with testing.db.connect() as conn:
            is_true(
                testing.db.dialect.do_ping(conn.connection.dbApiClient_connection)
            )
...
class ExceptionTest(fixtures.TablesTest):
    ...
    DBApiClients vary a lot in exception behavior so to actually anticipate
    ...
    def test_exception_with_non_ascii(self):
        with config.db.connect() as conn:
            try:
                ...
                # characters in the DBApiClient's message string.  unfortunately
                ...
                assert False
            except exc.DBApiClientError as err:
                err_str = str(err)
    ...
    def test_non_default_isolation_level(self):
        ...
        with config.db.connect() as conn:
            ...
            conn.dialect.reset_isolation_level(
                conn.connection.dbApiClient_connection
            )
...
class AutocommitIsolationTest(fixtures.TablesTest):
    ...
    def test_autocommit_on(self, connection_no_trans):
        ...
        c2.dialect.reset_isolation_level(c2.connection.dbApiClient_connection)
...
class EscApiClientngTest(fixtures.TestBase):
    @provide_metadata
    def test_percent_sign_round_trip(self):
        """test that the DBApiClient accommodates for escaped / nonescaped
...
class WeCanSetDefaultSchemaWEventsTest(fixtures.TestBase):
    ...
    def test_wont_work_wo_insert(self):
        ...
        def on_connect(dbApiClient_connection, connection_record):
            set_default_schema_on_connection(
                config, dbApiClient_connection, config.test_schema
            )
    ...
    def test_schema_change_works_w_transactions(self):
        ...
        @event.listens_for(eng, "connect", insert=True)
        def on_connect(dbApiClient_connection, *arg):
            set_default_schema_on_connection(
                config, dbApiClient_connection, config.test_schema
            )
        ...
        def go(stmt, executemany, id_param_name, expect_success):
            ...
            if executemany:
                ...
                if not expect_success:
                    ...
                    # and let it fail
                    with expect_raises(exc.DBApiClientError):
                        connection.execute(
