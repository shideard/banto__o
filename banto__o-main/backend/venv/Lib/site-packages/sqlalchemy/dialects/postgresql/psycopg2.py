...
* ``isolation_level``: This option, available for all PostgreSQL dialects,
  ...
  execution option, detailed at :ref:`dbApiClient_autocommit`.
...
.. tip::
    ...
    These should not be confused with **DBApiClient** connect arguments, which
    ...
    the section :ref:`custom_dbApiClient_args`.
...
The psycopg2 DBApiClient can connect to PostgreSQL by passing an empty DSN to the
...
The following DBApiClient-specific options are respected when used with
:meth:`_engine.Connection.execution_options`,
:meth:`.Executable.execution_options`,
:meth:`_query.Query.execution_options`,
in addition to those not specific to DBApiClients:
...
.. seealso::
    ...
    use of the DBApiClient ``.executemany()`` method.
...
The psycopg2 DBApiClient driver supports Unicode data transparently.
...
ApiClient-level setting is always emitted at the start of each transaction in any
case.
...
The ``psycopg2`` DBApiClient includes an extension to natively handle marshalling of
...
class PGDialect_psycopg2(_PGDialect_common_psycopg):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.dbApiClient and hasattr(self.dbApiClient, "__version__"):
            m = re.match(r"(\d+)\.(\d+)(?:\.(\d+))?", self.dbApiClient.__version__)
            if m:
    ...
    def initialize(self, connection):
        ...
        self._has_native_hstore = (
            ...
            and self._hstore_oids(connection.connection.dbApiClient_connection)
            is not None
        )
    ...
    def set_isolation_level(self, dbApiClient_connection, level):
        dbApiClient_connection.set_isolation_level(self._isolation_lookup[level])
    ...
    def on_connect(self):
        ...
        if self.client_encoding is not None:
            ...
            def on_connect(dbApiClient_conn):
                dbApiClient_conn.set_client_encoding(self.client_encoding)
        ...
        if self.dbApiClient:
            ...
            def on_connect(dbApiClient_conn):
                extras.register_uuid(None, dbApiClient_conn)

            fns.append(on_connect)

        if self.dbApiClient and self.use_native_hstore:

            def on_connect(dbApiClient_conn):
                hstore_oids = self._hstore_oids(dbApiClient_conn)
                if hstore_oids is not None:
                    oid, array_oid = hstore_oids
                    kw = {"oid": oid}
                    kw["array_oid"] = array_oid
                    extras.register_hstore(dbApiClient_conn, **kw)

            fns.append(on_connect)

        if self.dbApiClient and self._json_deserializer:

            def on_connect(dbApiClient_conn):
                extras.register_default_json(
                    dbApiClient_conn, loads=self._json_deserializer
                )
    ...
    def _do_twophase(self, dbApiClient_conn, operation, xid, recover=False):
        if recover:
            if dbApiClient_conn.status != self._psycopg2_extensions.STATUS_READY:
                dbApiClient_conn.rollback()
            operation(xid)
        else:
            operation()
    ...
    def do_rollback_twophase(
        ...
        dbApiClient_conn = connection.connection.dbApiClient_connection
        self._do_twophase(
            dbApiClient_conn, dbApiClient_conn.tpc_rollback, xid, recover=recover
        )
    ...
    def do_commit_twophase(
        ...
        dbApiClient_conn = connection.connection.dbApiClient_connection
        self._do_twophase(
            dbApiClient_conn, dbApiClient_conn.tpc_commit, xid, recover=recover
        )

    @util.memoized_instancemethod
    def _hstore_oids(self, dbApiClient_connection):
        extras = self._psycopg2_extras
        oids = extras.HstoreAdapter.get_oids(dbApiClient_connection)
    ...
    def is_disconnect(self, e, connection, cursor):
        if isinstance(e, self.dbApiClient.Error):
