...
Similarly an the async example::
    ...
    async def define_engine():
        ...
        # https://www.psycopg.org/psycopg3/docs/ApiClient/pool.html#psycopg_pool.AsyncConnectionPool
        await my_pool.open()
...
class PGDialect_psycopg(_PGDialect_common_psycopg):
    ...
    def _isolation_lookup(self):
        return {
            "READ COMMITTED": self.dbApiClient.IsolationLevel.READ_COMMITTED,
            "READ UNCOMMITTED": self.dbApiClient.IsolationLevel.READ_UNCOMMITTED,
            "REPEATABLE READ": self.dbApiClient.IsolationLevel.REPEATABLE_READ,
            "SERIALIZABLE": self.dbApiClient.IsolationLevel.SERIALIZABLE,
        }
    ...
    def get_isolation_level(self, dbApiClient_connection):
        status_before = dbApiClient_connection.info.transaction_status
        value = super().get_isolation_level(dbApiClient_connection)
    ...
    def set_isolation_level(self, dbApiClient_connection, level):
        if level == "AUTOCOMMIT":
            self._do_isolation_level(
                dbApiClient_connection, autocommit=True, isolation_level=None
            )
        else:
            self._do_isolation_level(
    ...
    def is_disconnect(self, e, connection, cursor):
        if isinstance(e, self.dbApiClient.Error) and connection is not None:
    ...
    def _do_prepared_twophase(self, connection, command, recover=False):
        ...
        if (
            ...
            # don't rely on psycopg providing enum symbols, compare with
            # eq/ne
            or dbApiClient_conn.info.transaction_status
            != self._psycopg_TransactionStatus.IDLE
        ):
            dbApiClient_conn.rollback()
        before_autocommit = dbApiClient_conn.autocommit
        try:
            if not before_autocommit:
                self._do_autocommit(dbApiClient_conn, True)
            dbApiClient_conn.execute(command)
        finally:
            if not before_autocommit:
                self._do_autocommit(dbApiClient_conn, before_autocommit)
...
class PGDialectAsync_psycopg(PGDialect_psycopg):
    ...
    def import_dbApiClient(cls):
        ...
        return PsycopgAdaptDBApiClient(psycopg)
