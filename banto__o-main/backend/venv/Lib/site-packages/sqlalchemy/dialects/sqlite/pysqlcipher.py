...
.. dialect:: sqlite+pysqlcipher
    :name: pysqlcipher
    :dbApiClient: sqlcipher 3 or pysqlcipher
    :connectstring: sqlite+pysqlcipher://:passphrase@/file_path[?kdf_iter=<iter>]

    Dialect for support of DBApiClients that make use of the
...
* If the :paramref:`_sa.create_engine.module` parameter supplies a DBApiClient module,
  that module is used.
...
.. warning:: The ``pysqlcipher3`` and ``pysqlcipher`` DBApiClient drivers are no
   longer maintained; the ``sqlcipher3`` driver as of this writing appears
   to be current.  For future compatibility, any pysqlcipher-compatible DBApiClient
...
as documented at https://www.zetetic.net/sqlcipher/sqlcipher-ApiClient/ can be passed
...
class SQLiteDialect_pysqlcipher(SQLiteDialect_pysqlite):
    ...
    def import_dbApiClient(cls):
        ...
        from pysqlcipher3 import dbApiClient2 as sqlcipher
