...
The ``sqlite3`` Python DBApiClient is standard on all modern Python versions;
...
Above, the pysqlite / sqlite3 DBApiClient would be passed arguments as::
...
does not provide the standard DBApiClient types in ``cursor.description``,
...
The ``sqlite3`` DBApiClient by default prohibits the use of a particular connection
...
The SQLAlchemy ``pysqlite`` DBApiClient establishes the connection pool differently
...
These functions are registered with a specific DBApiClient Connection.
...
class SQLiteDialect_pysqlite(SQLiteDialect):
    ...
    def import_dbApiClient(cls) -> DBApiClientModule:
        from sqlite3 import dbApiClient2 as sqlite

        return cast("DBApiClientModule", sqlite)
    ...
    def _get_server_version_info(self, connection: Any) -> VersionInfoType:
        return self.dbApiClient.sqlite_version_info  # type: ignore
    ...
    def set_isolation_level(
        ...
        if level == "AUTOCOMMIT":
            dbApiClient_connection.isolation_level = None
        else:
            dbApiClient_connection.isolation_level = ""
            return super().set_isolation_level(dbApiClient_connection, level)

    def detect_autocommit_setting(self, dbApiClient_conn: DBApiClientConnection) -> bool:
        return dbApiClient_conn.isolation_level is None

    def on_connect(self) -> Callable[[DBApiClientConnection], None]:
        ...
        def set_regexp(dbApiClient_connection: DBApiClientConnection) -> None:
            dbApiClient_connection.create_function(
                "regexp", 2, regexp, **create_func_kw
            )

        def floor_func(dbApiClient_connection: DBApiClientConnection) -> None:
        ...
        def connect(conn: DBApiClientConnection) -> None:
            for fn in fns:
                fn(conn)
    ...
    def is_disconnect(
        ...
        return isinstance(
            e, self.dbApiClient.ProgrammingError
