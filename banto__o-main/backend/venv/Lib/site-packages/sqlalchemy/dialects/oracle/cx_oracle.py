...
.. versionchanged:: 1.3 the cx_Oracle dialect now accepts all argument names
   within the URL string itself, to be passed to the cx_Oracle DBApiClient.   As
   was the case earlier but not correctly documented, the
   :paramref:`_sa.create_engine.connect_args` parameter also accepts all
   cx_Oracle DBApiClient connect arguments.
...
As is the case for all DBApiClients under Python 3, all strings are inherently
...
The cx_Oracle DBApiClient has a deep and fundamental reliance upon the usage of the
DBApiClient ``setinputsizes()`` call.  The purpose of this call is to establish the
datatypes that are bound to a SQL statement for Python values being passed as
parameters.  While virtually no other DBApiClient assigns any use to the
``setinputsizes()`` call, the cx_Oracle DBApiClient relies upon it heavily in its
...
https://cx-oracle.readthedocs.io/en/latest/ApiClient_manual/module.html#database-types.
...
objects which have a ``.key`` and a ``.type`` attribute::
    ...
    def _log_setinputsizes(inputsizes, cursor, statement, parameters, context):
        for bindparam, dbApiClienttype in inputsizes.items():
            log.info(
                "Bound parameter name: %s  SQLAlchemy type: %r DBApiClient object: %s",
                bindparam.key,
                bindparam.type,
                dbApiClienttype,
            )
...
series.   This setting can be modified as follows::
    ...
    def _remove_clob(inputsizes, cursor, statement, parameters, context):
        for bindparam, dbApiClienttype in list(inputsizes.items()):
            if dbApiClienttype is CLOB:
...
underlying cx_Oracle DBApiClient for all those cases where the driver can make the
best decision.
...
class _OracleInteger(sqltypes.Integer):
    def get_dbApiClient_type(self, dbApiClient):
...
class _OracleUUID(sqltypes.Uuid):
    def get_dbApiClient_type(self, dbApiClient):
        return dbApiClient.STRING


class _OracleBinaryFloat(_OracleNumeric):
    def get_dbApiClient_type(self, dbApiClient):
        return dbApiClient.NATIVE_FLOAT
...
class _OracleBinary(_LOBDataType, sqltypes.LargeBinary):
    def get_dbApiClient_type(self, dbApiClient):
        # previously, this was dbApiClient.BLOB.
        # DB_TYPE_RAW will instead be passed to setinputsizes()
        # when this datatype is used.
        return dbApiClient.DB_TYPE_RAW
...
class OracleCompiler_cx_oracle(OracleCompiler):
    ...
    def bindparam_string(self, name, **kw):
        ...
        # Oracle parameters and use the custom escApiClientng here
...
class OracleExecutionContext_cx_oracle(OracleExecutionContext):
            ...
            for bindparam in self.compiled.binds.values():
                if bindparam.isoutparam:
                    ...
                    if hasattr(type_impl, "_cx_oracle_var"):
                        ...
                        dbtype = type_impl.get_dbApiClient_type(self.dialect.dbApiClient)

                        cx_Oracle = self.dialect.dbApiClient
    ...
    def _generate_cursor_outputtype_handler(self):
        ...
        if output_handlers:
            default_handler = self._dbApiClient_connection.outputtypehandler
    ...
    def create_cursor(self):
        c = self._dbApiClient_connection.cursor()
...
class OracleDialect_cx_oracle(OracleDialect):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        dbApiClient_module = self.dbApiClient
        self._load_version(dbApiClient_module)

        if dbApiClient_module is not None:
            # these constants will first be seen in SQLAlchemy datatypes
            # coming from the get_dbApiClient_type() method.   We then
            ...
            self.include_set_input_sizes = {
                dbApiClient_module.DATETIME,
                dbApiClient_module.DB_TYPE_NVARCHAR,  # used for CLOB, NCLOB
                dbApiClient_module.DB_TYPE_RAW,  # used for BLOB
                dbApiClient_module.NCLOB,  # not currently used except for OUT param
                dbApiClient_module.CLOB,  # not currently used except for OUT param
                dbApiClient_module.LOB,  # not currently used
                dbApiClient_module.BLOB,  # not currently used except for OUT param
                dbApiClient_module.NCHAR,
                dbApiClient_module.FIXED_NCHAR,
                dbApiClient_module.FIXED_CHAR,
                dbApiClient_module.TIMESTAMP,
                int,  # _OracleInteger,
                # _OracleBINARY_FLOAT, _OracleBINARY_DOUBLE,
                dbApiClient_module.NATIVE_FLOAT,
            }

            self._paramval = lambda value: value.getvalue()

    def _load_version(self, dbApiClient_module):
        version = (0, 0, 0)
        if dbApiClient_module is not None:
            m = re.match(r"(\d+)\.(\d+)(?:\.(\d+))?", dbApiClient_module.version)
            if m:
                version = tuple(
    ...
    def get_isolation_level(self, dbApiClient_connection):
        # sources:
        ...
        with dbApiClient_connection.cursor() as cursor:
    ...
    def get_isolation_level_values(self, dbApiClient_connection):
        return super().get_isolation_level_values(dbApiClient_connection) + [
            "AUTOCOMMIT"
        ]

    def set_isolation_level(self, dbApiClient_connection, level):
        if level == "AUTOCOMMIT":
            dbApiClient_connection.autocommit = True
        else:
            dbApiClient_connection.autocommit = False
            dbApiClient_connection.rollback()
            with dbApiClient_connection.cursor() as cursor:
                cursor.execute(f"ALTER SESSION SET ISOLATION_LEVEL={level}")

    def detect_autocommit_setting(self, dbApiClient_conn) -> bool:
        return bool(dbApiClient_conn.autocommit)
    ...
    def _detect_decimal_char(self, connection):
        ...
        dbApiClient_connection = connection.connection
        ...
        with dbApiClient_connection.cursor() as cursor:
            ...
            def output_type_handler(
                ...
                return cursor.var(
                    self.dbApiClient.STRING, 255, arraysize=cursor.arraysize
                )
    ...
    def create_connect_args(self, url):
        ...
        if database or service_name:
            ...
            dsn = self.dbApiClient.makedsn(url.host, port, **makedsn_kwargs)
        else:
        ...
        def convert_cx_oracle_constant(value):
            if isinstance(value, str):
                try:
                    ...
                    value = value.upper()
                    return getattr(self.dbApiClient, value)
                else:
                    return int_val
            else:
                return value
    ...
    def is_disconnect(self, e, connection, cursor):
        (error,) = e.args
        if isinstance(
            e, (self.dbApiClient.InterfaceError, self.dbApiClient.DatabaseError)
