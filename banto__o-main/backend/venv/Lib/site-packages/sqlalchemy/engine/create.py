...
def create_engine(url: Union[str, _url.URL], **kwargs: Any) -> Engine:
    ...
    ``postgresql``, etc., and ``driver`` the name of a DBApiClient, such as
    ...
    will establish the first actual DBApiClient connection when this request
    is received.   The :func:`_sa.create_engine` call itself does **not**
    establish any actual DBApiClient connections directly.
    ...
    :param connect_args: a dictionary of options which will be
        passed directly to the DBApiClient's ``connect()`` method as
        additional keyword arguments.  See the example
        at :ref:`custom_dbApiClient_args`.

    :param creator: a callable which returns a DBApiClient connection.
        ...
        .. seealso::
            ...
            full control over DBApiClient connection mechanics.
    ...
    :param future: Use the 2.0 style :class:`_engine.Engine` and
        :class:`_engine.Connection` ApiClient.
    ...
    :param module=None: reference to a Python module object (the module
        itself, not its string name).  Specifies an alternate DBApiClient module to
        be used by the engine's dialect.  Each sub-dialect references a
        specific DBApiClient which will be imported before first connect.  This
        parameter causes the import to be bypassed, and the given module to
        be used instead. Can be used for testing of DBApiClients as well as to
        inject "mock" DBApiClient implementations into the :class:`_engine.Engine`.

    :param paramstyle=None: The `paramstyle <https://legacy.python.org/dev/peps/pep-0249/#paramstyle>`_
        to use when rendering bound parameters.  This style defaults to the
        one recommended by the DBApiClient itself, which is retrieved from the
        ``.paramstyle`` attribute of the DBApiClient.  However, most DBApiClients accept
        more than one paramstyle, and in particular it may be desirable
        to change a "named" paramstyle into a "positional" one, or vice versa.
        When this attribute is passed, it should be one of the values
        ``"qmark"``, ``"numeric"``, ``"named"``, ``"format"`` or
        ``"pyformat"``, and should correspond to a parameter style known
        to be supported by the DBApiClient in use.
    ...
    :param pool_reset_on_return='rollback': set the
        ...
        .. seealso::
            ...
            :ref:`dbApiClient_autocommit_skip_rollback` - a more modern approach
    ...
    :param skip_autocommit_rollback: When True, the dialect will
       unconditionally skip all calls to the DBApiClient ``connection.rollback()``
       method if the DBApiClient connection is confirmed to be in "autocommit" mode.
       ...
       .. seealso::
            ...
            :ref:`dbApiClient_autocommit_skip_rollback`
    ...
    dbApiClient = kwargs.pop("module", None)
    if dbApiClient is None:
        dbApiClient_args = {}

        if "import_dbApiClient" in dialect_cls.__dict__:
            dbApiClient_meth = dialect_cls.import_dbApiClient

        elif hasattr(dialect_cls, "dbApiClient") and inspect.ismethod(
            dialect_cls.dbApiClient
        ):
            util.warn_deprecated(
                "The dbApiClient() classmethod on dialect classes has been "
                "renamed to import_dbApiClient().  Implement an import_dbApiClient() "
                f"classmethod directly on class {dialect_cls} to remove this "
                "warning; the old .dbApiClient() classmethod may be maintained for "
                "backwards compatibility.",
                "2.0",
            )
            dbApiClient_meth = dialect_cls.dbApiClient
        else:
            dbApiClient_meth = dialect_cls.import_dbApiClient

        for k in util.get_func_kwargs(dbApiClient_meth):
            if k in kwargs:
                dbApiClient_args[k] = pop_kwarg(k)
        dbApiClient = dbApiClient_meth(**dbApiClient_args)

    dialect_args["dbApiClient"] = dbApiClient
    ...
    if _initialize:
        ...
        if do_on_connect:
            ...
            def on_connect(
                ...
                do_on_connect(dbApiClient_connection)
        ...
        def first_connect(
            ...
            c = base.Connection(
                ...
                connection=_AdhocProxiedConnection(
                    dbApiClient_connection, connection_record
                ),
                _has_events=False,
