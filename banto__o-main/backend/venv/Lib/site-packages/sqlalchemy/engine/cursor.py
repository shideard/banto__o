...
class CursorResultMetaData(ResultMetaData):
    """Result metadata for DBApiClient cursors."""
    ...
    def _colnames_from_description(
        ...
        context: DefaultExecutionContext,
        cursor_description: _DBApiClientCursorDescription,
    ) -> Iterator[Tuple[int, str, Optional[str], DBApiClientType]]:
...
class ResultFetchStrategy:
    ...
    alternate_cursor_description: Optional[_DBApiClientCursorDescription] = None

    def soft_close(
        self, result: CursorResult[Any], dbApiClient_cursor: Optional[DBApiClientCursor]
    ) -> None:
    ...
    def yield_per(
        ...
        dbApiClient_cursor: DBApiClientCursor,
        num: int,
    ) -> None:
        return
    ...
    def handle_exception(
        self,
        result: CursorResult[Any],
        dbApiClient_cursor: Optional[DBApiClientCursor],
        err: BaseException,
    ) -> NoReturn:
        raise err
...
class NoCursorFetchStrategy(ResultFetchStrategy):
    ...
    def fetchall(
        self, result: CursorResult[Any], dbApiClient_cursor: DBApiClientCursor
    ) -> Any:
        return self._non_result(result, [])
...
class CursorFetchStrategy(ResultFetchStrategy):
    """Call fetch methods from a DBApiClient cursor.
    ...
    def handle_exception(
        ...
        result.connection._handle_dbApiClient_exception(
            err, None, None, dbApiClient_cursor, result.context
        )

    def yield_per(
        self, result: CursorResult[Any], dbApiClient_cursor: DBApiClientCursor, num: int
    ) -> None:
    ...
    def fetchone(
        ...
        try:
            row = dbApiClient_cursor.fetchone()
            if row is None:
                result._soft_close(hard=hard_close)
            return row
        except BaseException as e:
            self.handle_exception(result, dbApiClient_cursor, e)
    ...
    def fetchmany(
        ...
        try:
            if size is None:
                l = dbApiClient_cursor.fetchmany()
            else:
                l = dbApiClient_cursor.fetchmany(size)
    ...
    def fetchall(
        ...
        try:
            rows = dbApiClient_cursor.fetchall()
            result._soft_close()
            return rows
        except BaseException as e:
            self.handle_exception(result, dbApiClient_cursor, e)
...
class BufferedRowCursorFetchStrategy(CursorFetchStrategy):
    ...
    interfacing with a DB-ApiClient that requires rows to be consumed before
    ...
    def __init__(
        ...
        if initial_buffer is not None:
            ...
            self._rowbuffer = collections.deque(dbApiClient_cursor.fetchmany(1))
    ...
    def _buffer_rows(
        ...
        try:
            if size < 1:
                new_rows = dbApiClient_cursor.fetchall()
            else:
                new_rows = dbApiClient_cursor.fetchmany(size)
        except BaseException as e:
            self.handle_exception(result, dbApiClient_cursor, e)
    ...
    def soft_close(
        ...
        super().soft_close(result, dbApiClient_cursor)

    def hard_close(
        self, result: CursorResult[Any], dbApiClient_cursor: Optional[DBApiClientCursor]
    ) -> None:
        self._rowbuffer.clear()
        super().hard_close(result, dbApiClient_cursor)
    ...
    def fetchone(
        ...
        if not self._rowbuffer:
            self._buffer_rows(result, dbApiClient_cursor)
            if not self._rowbuffer:
                try:
                    result._soft_close(hard=hard_close)
                except BaseException as e:
    ...
    def fetchmany(
        ...
        if size is None:
            return self.fetchall(result, dbApiClient_cursor)
        ...
        if size > lb:
            try:
                new = dbApiClient_cursor.fetchmany(size - lb)
            except BaseException as e:
                self.handle_exception(result, dbApiClient_cursor, e)
            else:
                if not new:
    ...
    def fetchall(
        ...
        try:
            ret = list(self._rowbuffer) + list(dbApiClient_cursor.fetchall())
            self._rowbuffer.clear()
            result._soft_close()
            return ret
        except BaseException as e:
...
class FullyBufferedCursorFetchStrategy(CursorFetchStrategy):
    ...
    def __init__(
        self,
        dbApiClient_cursor: Optional[DBApiClientCursor],
        alternate_description: Optional[_DBApiClientCursorDescription] = None,
        ...
        if initial_buffer is not None:
            ...
            assert dbApiClient_cursor is not None
            self._rowbuffer = collections.deque(dbApiClient_cursor.fetchall())
...
class CursorResult(Result[_T]):
    """A Result that is representing state from a DBApiClient cursor.

    .. versionchanged:: 1.4  The :class:`.CursorResult``
       class replaces the previous :class:`.ResultProxy` interface.
       This classes are based on the :class:`.Result` calling ApiClient
       which provides an updated usage model and calling facade for
       SQLAlchemy Core and SQLAlchemy ORM.

    Returns database rows via the :class:`.Row` class, which provides
    additional ApiClient features and behaviors on top of the raw data returned by
    the DBApiClient.   Through the use of filters such as the :meth:`.Result.scalars`
    ...
    def __init__(
        ...
        context: DefaultExecutionContext,
        cursor_strategy: ResultFetchStrategy,
        cursor_description: Optional[_DBApiClientCursorDescription],
    ):
    ...
    def _soft_close(self, hard: bool = False) -> None:
        ...
        This releases all DBApiClient cursor resources, but leaves the
    ...
    def rowcount(self) -> int:
        ...
        of the DBApiClient before the cursor is closed, to support DBApiClients that
        don't make this value available after cursor close.   Some DBApiClients may
        ...
        For cases where the DBApiClient does not support rowcount for a particular
        kind of statement and/or execution, the returned value will be ``-1``,
        which is delivered directly from the DBApiClient and is part of :pep:`249`.
        All DBApiClients should support rowcount for single-parameter-set
        ...
        .. note::
           ...
           * :attr:`_engine.CursorResult.rowcount` in the default case is
             *only* useful in conjunction with an UPDATE or DELETE statement,
             ...
             DBApiClients do not support rowcount values for statements that are not
             UPDATE or DELETE, particularly when rows are being returned which
             are not fully pre-buffered.   DBApiClients that dont support rowcount
           ...
           * :attr:`_engine.CursorResult.rowcount` may not be meaningful
             ...
             (i.e. an :term:`executemany`). Most DBApiClients do not sum "rowcount"
    ...
    def lastrowid(self) -> int:
        """Return the 'lastrowid' accessor on the DBApiClient cursor.

        This is a DBApiClient specific method and is only functional
    ...
    def returns_rows(self) -> bool:
        ...
        always be synonymous with whether or not the DBApiClient cursor had a
    ...
    def close(self) -> None:
        ...
        This closes out the underlying DBApiClient cursor corresponding to the
        statement execution, if one is still present.  Note that the DBApiClient
