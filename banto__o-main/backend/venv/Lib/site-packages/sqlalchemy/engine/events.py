...
class ConnectionEvents(event.Events[ConnectionEventsTarget]):
    ...
    to the DBApiClient ``cursor`` in the connection's :class:`.Dialect`.
    ...
    def before_cursor_execute(
        ...
        statement: str,
        parameters: _DBApiClientAnyExecuteParams,
        context: Optional[ExecutionContext],
        executemany: bool,
    ) -> Optional[Tuple[str, _DBApiClientAnyExecuteParams]]:
        """Intercept low-level cursor execute() events before execution,
        receiving the string SQL statement and DBApiClient-specific parameter list to
        ...
        :param conn: :class:`_engine.Connection` object
        :param cursor: DBApiClient cursor object
        :param statement: string SQL statement, as to be passed to the DBApiClient
        :param parameters: Dictionary, tuple, or list of parameters being
         passed to the ``execute()`` or ``executemany()`` method of the
         DBApiClient ``cursor``.  In some cases may be ``None``.
    ...
    def after_cursor_execute(
        ...
        :param conn: :class:`_engine.Connection` object
        :param cursor: DBApiClient cursor object.  Will have results pending
         if the statement was a SELECT, but these should not be consumed
         as they will be needed by the :class:`_engine.CursorResult`.
        :param statement: string SQL statement, as passed to the DBApiClient
        :param parameters: Dictionary, tuple, or list of parameters being
         passed to the ``execute()`` or ``executemany()`` method of the
         DBApiClient ``cursor``.  In some cases may be ``None``.
        :param context: :class:`.ExecutionContext` object in use.  May
    ...
    def engine_connect(self, conn: Connection) -> None:
        ...
        refers to the actual connection to a database at the DBApiClient level;
        a DBApiClient connection may be pooled and reused for many operations.
        In contrast, this event refers only to the production of a higher level
        :class:`_engine.Connection` wrapper around such a DBApiClient connection.
        ...
        not the
        DBApiClient connection that :meth:`_events.PoolEvents.checkout` deals with,
        although
        this DBApiClient connection is available here via the
        ...
        .. seealso::
            ...
            for an individual DBApiClient connection
    ...
    def rollback(self, conn: Connection) -> None:
        ...
        a DBApiClient connection upon checkin, if the ``reset_on_return``
...
class DialectEvents(event.Events[Dialect]):
    ...
    of key dialect functions which interact with the DBApiClient.
    ...
    .. note::
        ...
        where intricate re-statement of DBApiClient mechanics must be injected onto
    ...
    def handle_error(
        ...
        This includes all exceptions emitted by the DBApiClient as well as
        ...
        * read-only, low-level exception handling for logging and
          debugging purposes
        * Establishing whether a DBApiClient connection error message indicates
        ...
        * exception re-writing
        ...
        make use of a native ``ping()`` method supplied by the DBApiClient which does
    ...
    def do_connect(
    ...
    ) -> Optional[DBApiClientConnection]:
        """Receive connection arguments before a connection is made.

        This event is useful in that it allows the handler to manipulate the
        cargs and/or cparams collections that control how the DBApiClient
        ...
        entirely, by returning a non-``None`` DBApiClient connection object::
    ...
    def do_execute_no_params(
        self, cursor: DBApiClientCursor, statement: str, context: ExecutionContext
    ) -> Optional[Literal[True]]:
    ...
    def do_setinputsizes(
        ...
        DBApiClient ``cursor.setinputsizes()`` method which passes information about
        parameter binding for a particular statement.   The given
        ``inputsizes`` dictionary will contain :class:`.BindParameter` objects
        as keys, linked to DBApiClient-specific type objects as values; for
        ...
        order to make decisions about the DBApiClient object.
        ...
        or a dictionary of string parameter keys to DBApiClient type objects for
