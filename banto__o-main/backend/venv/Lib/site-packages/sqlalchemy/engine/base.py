...
from .interfaces import DBApiClientCursor
...
if typing.TYPE_CHECKING:
    ...
    from .interfaces import _DBApiClientAnyExecuteParams
    from .interfaces import _DBApiClientSingleExecuteParams
...
class Connection(ConnectionEventsTarget, inspection.Inspectable["Inspector"]):
    """Provides high-level functionality for a wrapped DB-ApiClient connection.
    ...
    possible that the underlying DBApiClient connection may not support shared
    access between threads. Check the DBApiClient documentation for details.

    The Connection object represents a single DBApiClient connection checked out
    ...
    _dbApiClient_connection: Optional[PoolProxiedConnection]
    ...
    def __init__(
        ...
        if connection is None:
            try:
                self._dbApiClient_connection = engine.raw_connection()
            except dialect.loaded_dbApiClient.Error as err:
                Connection._handle_dbApiClient_exception_noconnection(
                    err, dialect, engine
                )
                raise
        else:
            self._dbApiClient_connection = connection
    ...
    def execution_options(self, **opt: Any) -> Connection:
        ...
        DBApiClient connection in any case so there is no feasible means of
        ...
        :param isolation_level: Available on: :class:`_engine.Connection`,
          ...
          statements on the DBApiClient connection, and **necessarily affects the
          original Connection object overall**. The isolation level will remain
          at the given setting until explicitly changed, or when the DBApiClient
          ...
          event handler will emit additional statements on the DBApiClient connection
        ...
        :param no_parameters: Available on: :class:`_engine.Connection`,
          ...
          Some DBApiClients such as psycopg2 and mysql-python consider
          percent signs as significant only when parameters are
          present; this option allows code to generate SQL
          containing percent signs (and possibly other characters)
          that is neutral regarding whether it's executed by the DBApiClient
        ...
        :param preserve_rowcount: Boolean; when True, the ``cursor.rowcount``
          ...
          statements.  Using this option, the DBApiClients rowcount value can
          be accessed for other kinds of statements such as INSERT and SELECT,
          to the degree that the DBApiClient supports these statements.  See
    ...
    @property
    def _still_open_and_dbApiClient_connection_is_valid(self) -> bool:
        pool_proxied_connection = self._dbApiClient_connection
        return (
    ...
    def closed(self) -> bool:
        ...
        return self._dbApiClient_connection is None and not self.__can_reconnect
    ...
    def invalidated(self) -> bool:
        ...
        # the _dbApiClient_connection would be None and closed=True, yet the
    ...
    def connection(self) -> PoolProxiedConnection:
        """The underlying DB-ApiClient connection managed by this Connection.

        This is a SQLAlchemy connection-pool proxied connection
        which then has the attribute
        :attr:`_pool._ConnectionFairy.dbApiClient_connection` that refers to the
        actual driver connection.

        .. seealso::


            :ref:`dbApiClient_connections`

        """

        if self._dbApiClient_connection is None:
            try:
                return self._revalidate_connection()
            except (exc.PendingRollbackError, exc.ResourceClosedError):
                raise
            except BaseException as e:
                self._handle_dbApiClient_exception(e, None, None, None, None)
        else:
            return self._dbApiClient_connection
    ...
    def get_isolation_level(self) -> IsolationLevel:
        ...
        is the actual level on the underlying DBApiClient connection regardless of
        ...
        .. note::  This method **will not report** on the ``AUTOCOMMIT``
          isolation level, which is a separate :term:`dbApiClient` setting that's
        ...
        """
        dbApiClient_connection = self.connection.dbApiClient_connection
        assert dbApiClient_connection is not None
        try:
            return self.dialect.get_isolation_level(dbApiClient_connection)
        except BaseException as e:
            self._handle_dbApiClient_exception(e, None, None, None, None)
    ...
    def _revalidate_connection(self) -> PoolProxiedConnection:
        if self.__can_reconnect and self.invalidated:
            ...
            self._dbApiClient_connection = self.engine.raw_connection()
            return self._dbApiClient_connection
        raise exc.ResourceClosedError("This Connection is closed")

    @property
    def info(self) -> _InfoType:
        """Info dictionary associated with the underlying DBApiClient connection
        referred to by this :class:`_engine.Connection`, allowing user-defined
        data to be associated with the connection.

        The data here will follow along with the DBApiClient connection including
    ...
    def invalidate(self, exception: Optional[BaseException] = None) -> None:
        """Invalidate the underlying DBApiClient connection associated with
        this :class:`_engine.Connection`.

        An attempt will be made to close the underlying DBApiClient connection
        ...
        procure a new DBApiClient connection using the services of the
        :class:`_pool.Pool` as a source of connectivity (e.g.
        a "reconnection").

        If a transaction was in progress (e.g. the
        :meth:`_engine.Connection.begin` method has been called) when
        :meth:`_engine.Connection.invalidate` method is called, at the DBApiClient
        level all state associated with this transaction is lost, as
        the DBApiClient connection is closed.  The :class:`_engine.Connection`
        ...
        if self._still_open_and_dbApiClient_connection_is_valid:
            pool_proxied_connection = self._dbApiClient_connection
            assert pool_proxied_connection is not None
            pool_proxied_connection.invalidate(exception)

        self._dbApiClient_connection = None

    def detach(self) -> None:
        """Detach the underlying DB-ApiClient connection from its connection pool.
        ...
        the DB-ApiClient connection will be literally closed and not
    ...
    def begin(self) -> RootTransaction:
        ...
        DBApiClient connection in any way; the Python DBApiClient does not have any
    ...
    def _begin_impl(self, transaction: RootTransaction) -> None:
        if self._echo:
            if self._is_autocommit_isolation():
                self._log_info(
                    "BEGIN (implicit; DBApiClient should not BEGIN due to "
                    "autocommit mode)"
                )
            else:
            ...
            if self._echo:
                if self._is_autocommit_isolation():
                    if self.dialect.skip_autocommit_rollback:
                        ...
                        self._log_info(
                            "ROLLBACK using DBApiClient connection.rollback(); "
    ...
    def _commit_impl(self) -> None:
        ...
        if self._echo:
            if self._is_autocommit_isolation():
                self._log_info(
                    "COMMIT using DBApiClient connection.commit(), "
    ...
    def close(self) -> None:
        ...
        resources, that is, the DBApiClient connection referenced
        internally. The DBApiClient connection is typically restored
        ...
        the DBApiClient connection is also unconditionally released via
        the DBApiClient connection's ``rollback()`` method, regardless
        ...
        if self._dbApiClient_connection is not None:
            conn = self._dbApiClient_connection
            ...
            # _dbApiClient_connection would already be None, however usually
    ...
    def execute(
        ...
        *,
        ...
        :param parameters: parameters which will be bound into the statement.
         ...
         will make use of the DBApiClient ``cursor.executemany()`` method.
         When a single dictionary is passed, the DBApiClient ``cursor.execute()``
         method will be used.
    ...
    def exec_driver_sql(
        self,
        statement: str,
        parameters: Optional[_DBApiClientAnyExecuteParams] = None,
        execution_options: Optional[CoreExecuteOptionsParameter] = None,
    ) -> CursorResult[Any]:
        r"""Executes a string SQL statement on the DBApiClient cursor directly,
        without any SQL compilation steps.

        This can be used to pass any string directly to the
        ``cursor.execute()`` method of the DBApiClient in use.

        :param statement: The statement str to be executed.   Bound parameters
         must use the underlying DBApiClient's paramstyle, such as "qmark",
         "pyformat", "format", etc.
    ...
    def _execute_context(
        ...
        **kw: Any,
        ...
        try:
            ...
            self._handle_dbApiClient_exception(
    ...
    def _exec_single_context(
        ...
        """continue the _execute_context() method for a single DBApiClient
    ...
    def _exec_insertmany_context(
        ...
        operation, which will invoke DBApiClient
    ...
    def _cursor_execute(
        ...
        statement: str,
        parameters: _DBApiClientSingleExecuteParams,
    ...
    def _safe_close_cursor(self, cursor: DBApiClientCursor) -> None:
    ...
    def _handle_dbApiClient_exception(
        self,
        e: BaseException,
        ...
        if not self._is_disconnect:
            self._is_disconnect = (
                isinstance(e, self.dialect.loaded_dbApiClient.Error)
                and not self.closed
                and self.dialect.is_disconnect(
                    e,
                    self._dbApiClient_connection if not self.invalidated else None,
                    cursor,
                )
        ...
        if self._reentrant_error:
            raise exc.DBApiClientError.instance(
                statement,
                parameters,
                e,
                self.dialect.loaded_dbApiClient.Error,
        ...
        try:
            # non-DBApiClient error - if we already got a context,
            # or there's no string statement, don't wrap it
            should_wrap = isinstance(e, self.dialect.loaded_dbApiClient.Error) or (
            ...
            if should_wrap:
                sqlalchemy_exception = exc.DBApiClientError.instance(
                    statement,
                    parameters,
                    cast(Exception, e),
                    self.dialect.loaded_dbApiClient.Error,
            ...
            if should_wrap and context:
                context.handle_dbApiClient_exception(e)
            ...
            if self._is_disconnect:
                ...
                if not self.invalidated:
                    dbApiClient_conn_wrapper = self._dbApiClient_connection
                    assert dbApiClient_conn_wrapper is not None
                    if invalidate_pool_on_disconnect:
                        self.engine.pool._invalidate(dbApiClient_conn_wrapper, e)
                    self.invalidate(e)

    @classmethod
    def _handle_dbApiClient_exception_noconnection(
        cls,
        e: BaseException,
        dialect: Dialect,
        ...
        if is_disconnect is None:
            is_disconnect = isinstance(
                e, dialect.loaded_dbApiClient.Error
            ) and dialect.is_disconnect(e, None, None)

        should_wrap = isinstance(e, dialect.loaded_dbApiClient.Error)
        ...
        if should_wrap:
            sqlalchemy_exception = exc.DBApiClientError.instance(
                ...
                None,
                cast(Exception, e),
                dialect.loaded_dbApiClient.Error,
                hide_parameters=(
...
class Transaction(TransactionalContext):
    ...
    def rollback(self) -> None:
        ...
        * For a :class:`.TwoPhaseTransaction`, DBApiClient-specific methods for two
...
class RootTransaction(Transaction):
    ...
    non-transactional state is used to emit commands on the DBApiClient connection.
...
class NestedTransaction(Transaction):
    ...
    * The :meth:`.NestedTransaction.rollback` method corresponds to a
    ...
    .. seealso::
        ...
        :ref:`session_begin_nested` - ORM version of the SAVEPOINT ApiClient.
...
class Engine(
    ...
    def execution_options(self, **opt: Any) -> OptionEngine:
        ...
        * The logging configuration and logging_name is copied from the parent
        ...
        configuration is at :ref:`dbApiClient_autocommit_multiple`.
    ...
    def connect(self) -> Connection:
        ...
        and its underlying DBApiClient resources are returned to the connection pool.
    ...
    def raw_connection(self) -> PoolProxiedConnection:
        """Return a "raw" DBApiClient connection from the connection pool.

        The returned object is a proxied version of the DBApiClient
        connection object used by the underlying driver in use.
        The object will have all the same behavior as the real DBApiClient
        connection, except that its ``close()`` method will result in the
        connection being returned to the pool, rather than being closed
        for real.

        This method provides direct DBApiClient connection access for
        special situations when the ApiClient provided by
        :class:`_engine.Connection`
        is not needed.   When a :class:`_engine.Connection` object is already
        present, the DBApiClient connection is available using
