...
from .type_ApiClient import Emulated
from .type_ApiClient import NativeForEmulated  # noqa
from .type_ApiClient import to_instance as to_instance
from .type_ApiClient import TypeDecorator as TypeDecorator
from .type_ApiClient import TypeEngine as TypeEngine
from .type_ApiClient import TypeEngineMixin
from .type_ApiClient import Variant  # noqa
...
if TYPE_CHECKING:
    ...
    from .type_ApiClient import _BindProcessorType
    from .type_ApiClient import _ComparatorFactory
    from .type_ApiClient import _LiteralProcessorType
    from .type_ApiClient import _MatchedOnType
    from .type_ApiClient import _ResultProcessorType
...
class Unicode(String):
    ...
    DBApiClient itself. All modern DBApiClients accommodate non-ASCII strings but may have
    different methods of managing database encodings; if necessary, this
    encoding should be configured as detailed in the notes for the target DBApiClient
    ...
    does not produce bytestring objects nor does it accommodate a DBApiClient that
...
class Numeric(HasExpressionLookup, TypeEngine[_N]):
    ...
    .. note::
        ...
        is a product of the SQL datatype in use, the Python :term:`DBApiClient`
    ...
    def __init__(
        ...
        :param asdecimal: default True.  Return whether or not
          ...
          as floats.   Different DBApiClients send one or the other based on
          datatypes - the Numeric type will ensure that return values
          are one or the other across DBApiClients consistently.
        ...
        that the asdecimal setting is appropriate for the DBApiClient in use -
        when Numeric applies a conversion from Decimal->float or float->
        Decimal, this conversion incurs an additional performance overhead
        for all result columns received.

        DBApiClients that return Decimal natively (e.g. psycopg2) will have
        better accuracy and higher performance with a setting of ``True``,
        as the native translation to Decimal reduces the amount of floating-
        point issues at play, and the Numeric type itself doesn't need
        to apply any further conversions.  However, another DBApiClient which
    ...
    def result_processor(self, dialect, coltype):
        if self.asdecimal:
            if dialect.supports_native_decimal:
                # we're a "numeric", DBApiClient will give us Decimal directly
                return None
            else:
                # we're a "numeric", DBApiClient returns floats, convert.
...
class DateTime(
    ...
    module.  Most DBApiClients have built in support for the datetime
...
class _Binary(TypeEngine[bytes]):
    ...
    def bind_processor(self, dialect):
        if dialect.dbApiClient is None:
            return None

        DBApiClientBinary = dialect.dbApiClient.Binary

        def process(value):
            if value is not None:
                return DBApiClientBinary(value)
            else:
                return None
...
class LargeBinary(_Binary):
    ...
    PostgreSQL.  It also handles the necessary conversions for the DBApiClient.
...
class TupleType(TypeEngine[Tuple[Any, ...]]):
    ...
    def coerce_compared_value(
        ...
        if value is type_ApiClient._NO_VALUE_IN_LIST:
            return super().coerce_compared_value(op, value)
        else:
            return TupleType(
                *[
...
class UUID(Uuid[_UUID_RETURN], type_ApiClient.NativeForEmulated):
...
# back-assign to type_ApiClient
type_ApiClient.BOOLEANTYPE = BOOLEANTYPE
type_ApiClient.STRINGTYPE = STRINGTYPE
type_ApiClient.INTEGERTYPE = INTEGERTYPE
type_ApiClient.NULLTYPE = NULLTYPE
type_ApiClient.NUMERICTYPE = NUMERICTYPE
type_ApiClient.MATCHTYPE = MATCHTYPE
type_ApiClient.INDEXABLE = INDEXABLE = Indexable
type_ApiClient.TABLEVALUE = TABLEVALUE
type_ApiClient._resolve_value_to_type = _resolve_value_to_type
