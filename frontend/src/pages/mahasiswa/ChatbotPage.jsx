...
export default function ChatbotPage() {
  /* pesan yang ditampilkan di layar */
  const [messages, setMessages] = useState([]);
  /* tombol yang sedang aktif (hanya 1 set tombol aktif di bawah) */
  const [activeButtons, setActiveButtons] = useState(null); // { type: 'category'|'question', items }
  ...
  /* init: tampilkan greeting + pilihan kategori */
  useEffect(() => {
    setMessages([{ id: "greeting", from: "bot", text: GREETING }]);
    setActiveButtons({ type: "category", items: CATEGORY_ITEMS });
  }, []);
