...
const styles = `
  ...
  .stat-box.handled { border-color: #bfdbfe; background: #eff6ff; }
  ...
  .stat-box.handled .stat-box-num { color: #2563eb; }
  .stat-box.selesai  .stat-box-num { color: #15803d; }
  .stat-box.avg      .stat-box-num { color: #c2410c; }
  ...
  .completion-bar-fill { height: 100%; border-radius: 100px; background: linear-gradient(90deg, #3b82f6, #1d4ed8); transition: width 0.8s ease; }
...
// ─────────────────────────── FAKULTAS LIST ───────────────────────────────────
const FAKULTAS_LIST = [
  { id: 1, nama: "Fakultas MIPA / Ilmu Komputer" },
  { id: 2, nama: "Fakultas Pertanian" },
  { id: 3, nama: "Sekolah Bisnis" },
  { id: 4, nama: "Sekolah Vokasi" },
  { id: 5, nama: "Fakultas Kehutanan" },
];
...
export default function ProfilPage() {
  ...
  const [formFakultas, setFormFakultas] = useState(user?.fakultas_id || "");
  ...
  // ── Fetch tiket ──
  useEffect(() => {
    // Memanggil API tiket (disesuaikan dengan endpoint getAllTiket yang me-return tiket milik user jika rolenya mahasiswa)
  ...
  // ── Statistik (Asumsi semua tiket yang di-fetch adalah milik mahasiswa ini) ──
  const dibuat   = tickets.length;
  const selesai  = tickets.filter(t => t.status === "SELESAI").length;
  const aktif    = tickets.filter(t => t.status !== "SELESAI").length;
  const completion = dibuat > 0 ? Math.round((selesai / dibuat) * 100) : 0;
  
  // Ambil tiket aktif terbaru
  const recentAktif = tickets.filter(t => t.status !== "SELESAI").slice(0, 4);

  const namaFakultas = FAKULTAS_LIST.find(f => f.id === (user?.fakultas_id || formFakultas))?.nama || user?.fakultas || "—";
  ...
  const handleSaveProfil = async () => {
    ...
    try {
      ...
      await apiClient.patch("/auth/me", {
        nama: formNama.trim(),
        fakultas_id: formFakultas ? Number(formFakultas) : null,
      });
      setProfilMsg({ type: "success", text: "Profil berhasil diperbarui!" });
      setEditMode(false);
      const stored = JSON.parse(localStorage.getItem("banto_user") || "{}");
      localStorage.setItem("banto_user", JSON.stringify({ ...stored, nama: formNama.trim(), fakultas_id: formFakultas ? Number(formFakultas) : null }));
      ...
      <main className="profil-main">
        ...
        <div className="profil-grid">
                ...
                <div className="profil-info-list">
                  ...
                  <div className="profil-info-item">
                    ...
                    <div>
                      ...
                      <div className="profil-info-val">{user?.nim || "—"}</div>
                    </div>
                  </div>
                  <div className="profil-info-item">
                    <div className="profil-info-icon">🏢</div>
                    <div>
                      <div className="profil-info-label">Fakultas / Departemen</div>
                      <div className="profil-info-val">
                        {namaFakultas !== "—" ? namaFakultas : <span className="muted">Belum ditentukan</span>}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div>
            ...
            <div className="profil-card">
              <div className="profil-card-header">
                <span className="profil-card-title">Statistik Tiket Saya</span>
                  ...
                  <>
                    <div className="stat-row">
                      <div className="stat-box handled">
                        <div className="stat-box-num">{dibuat}</div>
                        <div className="stat-box-label">Tiket Dibuat</div>
                      </div>
                      ...
                      <div className="stat-box avg">
                        <div className="stat-box-num">{aktif}</div>
                        <div className="stat-box-label">Sedang Aktif</div>
                      </div>
                    </div>
                    ...
                    {dibuat > 0 && (
                      <div className="completion-wrap">
                        <div className="completion-label">
                          ...
                          <span style={{ color: "#2563eb" }}>{completion}%</span>
                        </div>
                ...
                <div>
                  ...
                  {loadingTiket ? (
                    ...
                    <div className="tiket-mini-empty">
                      {dibuat === 0
                        ? "📭 Kamu belum pernah membuat tiket"
                        : "✅ Semua tiketmu sudah selesai diproses!"}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="profil-card">
              <div className="profil-card-header">
                ...
                {!editMode && (
                  <button
                    ...
                    onClick={() => { setEditMode(true); setFormNama(user?.nama || ""); setFormFakultas(user?.fakultas_id || ""); }}
                  >
                    ✏️ Edit
                  </button>
                )}
              ...
              <div className="profil-card-body">
                ...
                <div className="form-group">
                  ...
                  <input type="text" className="form-input readonly" value={user?.nim || ""} readOnly />
                  <div className="form-hint">NIM dan identitas akademik tidak dapat diubah sendiri.</div>
                </div>
                <div className="form-group" style={{ marginBottom: editMode ? 18 : 0 }}>
                  <label className="form-label">Fakultas / Departemen</label>
                  {editMode ? (
                    <select
              ...
              <div className="profil-card-body">
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 0 0" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#94a3b8" }}>Akun mahasiswa aktif</div>
                  </div>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 12px", background: "#eff6ff", color: "#2563eb", borderRadius: 100, fontSize: 11, fontWeight: 700 }}>
                    ● Aktif
                  </span>
                </div>
              </div>
