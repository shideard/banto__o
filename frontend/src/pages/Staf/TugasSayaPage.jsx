...
const styles = `
  ...
  .tugas-stat-mini-icon { width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 20px; }

  .tugas-list { display: flex; flex-direction: column; gap: 16px; }
  .tugas-card { background: #fff; border: 1.5px solid #e2e8f0; border-radius: 14px; padding: 20px 24px; transition: box-shadow 0.18s; }
...
export default function TugasSayaPage() {
  ...
  const displayed = (tab === "aktif" ? aktif : selesai).filter(t =>
  ...
  const getActionLabel = (status) => {
    if (status === "DIKLAIM") return "Mulai Proses →";
    ...
    <>
      ...
      <main className="staf-main">
        ...
        <div className="staf-page-header">
          <h1>Tugas Saya</h1>
          <p>Kelola tiket yang sedang Anda tangani.</p>
        </div>

        {/* Toolbar */}
        <div className="tugas-toolbar">
          <div className="tugas-tabs">
            <button className={`tugas-tab ${tab === "aktif" ? "active" : ""}`} onClick={() => setTab("aktif")}>
              Aktif ({aktif.length})
            </button>
            <button className={`tugas-tab ${tab === "selesai" ? "active" : ""}`} onClick={() => setTab("selesai")}>
              Selesai ({selesai.length})
            </button>
          </div>
          <div className="tugas-search">
            <span>🔍</span>
            <input placeholder="Cari ID atau judul..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="state-center">Tidak ada tiket {tab === "aktif" ? "aktif" : "selesai"} ditemukan.</div>
        ) : (
            ...
            {displayed.map(t => (
              <div key={t.id} className={`tugas-card ${t.status === "REVISI" ? "urgent" : ""}`}>
                ...
                <div className="tugas-card-desc">
                  {t.pengajuan?.deskripsi || "Tidak ada deskripsi."}
                </div>
