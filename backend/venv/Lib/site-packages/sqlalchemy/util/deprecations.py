...
def became_legacy_20(
    ApiClient_name: str, alternative: Optional[str] = None, **kw: Any
) -> Callable[[_F], _F]:
    type_reg = re.match("^:(attr|func|meth):", ApiClient_name)
    if type_reg:
    ...
    if ":attr:" in ApiClient_name:
        ...
        if not attribute_ok:
            assert kw.get("enable_warnings") is False, (
                ...
                "enable_warnings=False." % ApiClient_name
            )

    if alternative:
        message += " " + alternative

    warning_cls = exc.LegacyApiClientWarning
