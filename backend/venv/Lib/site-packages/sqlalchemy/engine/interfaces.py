...
if TYPE_CHECKING:
    ...
    from .url import URL
    from ..connectors.asyncio import AsyncIODBApiClientConnection
    ...
    from ..sql.type_ApiClient import _TypeMemoDict
    from ..sql.type_ApiClient import TypeEngine
...
class ExecuteStyle(Enum):
    """indicates the :term:`DBApiClient` cursor method that will be used to invoke
    a statement."""
...
class DBApiClientModule(Protocol):
...
class DBApiClientConnection(Protocol):
    ...
    def cursor(self, *args: Any, **kwargs: Any) -> DBApiClientCursor: ...
...
class DBApiClientType(Protocol):
...
class DBApiClientCursor(Protocol):
    ...
    def execute(
        self,
        operation: Any,
        parameters: Optional[_DBApiClientSingleExecuteParams] = None,
    ) -> Any: ...

    def executemany(
        self,
        operation: Any,
        parameters: _DBApiClientMultiExecuteParams,
    ) -> Any: ...
...
_DBApiClientSingleExecuteParams = Union[Sequence[Any], _CoreSingleExecuteParams]

_DBApiClientMultiExecuteParams = Union[
    Sequence[Sequence[Any]], _CoreMultiExecuteParams
]
_DBApiClientAnyExecuteParams = Union[
    _DBApiClientMultiExecuteParams, _DBApiClientSingleExecuteParams
]
_DBApiClientCursorDescription = Sequence[
    Tuple[
        str,
        "DBApiClientType",
        Optional[int],
...
_AnySingleExecuteParams = _DBApiClientSingleExecuteParams
_AnyMultiExecuteParams = _DBApiClientMultiExecuteParams
_AnyExecuteParams = _DBApiClientAnyExecuteParams
...
class BindTyping(Enum):
    ...
    This is only implemented for DBApiClients that support this method and for which
...
class Dialect(EventTarget):
    """Define the behavior of a specific database and DB-ApiClient combination.
    ...
    name: str
    """identifying name for the dialect from a DBApiClient-neutral point of view
      (i.e. 'sqlite')
    """

    driver: str
    """identifying name for the dialect's DBApiClient"""

    dialect_description: str

    dbApiClient: Optional[DBApiClientModule]
    """A reference to the DBApiClient module object itself.

    SQLAlchemy dialects import DBApiClient modules using the classmethod
    :meth:`.Dialect.import_dbApiClient`. The rationale is so that any dialect
    module can be imported and used to generate SQL statements without the
    need for the actual DBApiClient driver to be installed.  Only when an
    :class:`.Engine` is constructed using :func:`.create_engine` does the
    DBApiClient get imported; at that point, the creation process will assign
    the DBApiClient module to this attribute.

    Dialects should therefore implement :meth:`.Dialect.import_dbApiClient`
    which will import the necessary module and return it, and then refer
    to ``self.dbApiClient`` in dialect code in order to refer to the DBApiClient module
    contents.

    .. versionchanged:: The :attr:`.Dialect.dbApiClient` attribute is exclusively
       used as the per-:class:`.Dialect`-instance reference to the DBApiClient
       module.   The previous not-fully-documented ``.Dialect.dbApiClient()``
       classmethod is deprecated and replaced by :meth:`.Dialect.import_dbApiClient`.

    """

    @util.non_memoized_property
    def loaded_dbApiClient(self) -> DBApiClientModule:
        """same as .dbApiClient, but is never None; will raise an error if no
        DBApiClient was set up.
    ...
    paramstyle: str
    """the paramstyle to be used (some DB-ApiClients support multiple
      paramstyles).
    """
    ...
    dbApiClient_exception_translation_map: Mapping[str, str] = util.EMPTY_DICT
    """A dictionary of names that will contain as values the names of
       ...
       DBApiClient has exception classes that aren't named as they are
    ...
    """set of DBApiClient type objects that should be included in
    ...
    """set of DBApiClient type objects that should be excluded in
    automatic cursor.setinputsizes() calls.
    ...
    def create_connect_args(self, url: URL) -> ConnectArgsType:
        """Build DB-ApiClient compatible connection arguments.

        Given a :class:`.URL` object, returns a tuple
        consisting of a ``(*args, **kwargs)`` suitable to send directly
        to the dbApiClient's connect function.   The arguments are sent to the
        :meth:`.Dialect.connect` method which then runs the DBApiClient-level
        ``connect()`` function.
    ...
    @classmethod
    def import_dbApiClient(cls) -> DBApiClientModule:
        """Import the DBApiClient module that is used by this dialect.

        The Python module object returned here will be assigned as an
        instance variable to a constructed dialect under the name
        ``.dbApiClient``.

        .. versionchanged:: 2.0  The :meth:`.Dialect.import_dbApiClient` class
           method is renamed from the previous method ``.Dialect.dbApiClient()``,
           which would be replaced at dialect instantiation time by the
           DBApiClient module itself, thus using the same name in two different ways.
           If a ``.Dialect.dbApiClient()`` classmethod is present on a third-party
    ...
    def has_table(
        ...
        **kw: Any,
        ...
        .. note:: This method is used internally by SQLAlchemy, and is
           ...
           implementation. It is **not** the public ApiClient for checking for table
    ...
    def do_begin(self, dbApiClient_connection: PoolProxiedConnection) -> None:
        """Provide an implementation of ``connection.begin()``, given a
        DB-ApiClient connection.

        The DBApiClient has no dedicated "begin" method and it is expected
        that transactions are implicit.  This hook is provided for those
        DBApiClients that might need additional help in this area.

        :param dbApiClient_connection: a DBApiClient connection, typically
    ...
    def do_rollback(self, dbApiClient_connection: PoolProxiedConnection) -> None:
        """Provide an implementation of ``connection.rollback()``, given
        a DB-ApiClient connection.
    ...
    def do_commit(self, dbApiClient_connection: PoolProxiedConnection) -> None:
    ...
    def do_terminate(self, dbApiClient_connection: DBApiClientConnection) -> None:
        """Provide an implementation of ``connection.close()`` that tries as
        much as possible to not block, given a DBApiClient
        connection.

        In the vast majority of cases this just calls .close(), however
        for some asyncio dialects may call upon different ApiClient features.
    ...
    def do_close(self, dbApiClient_connection: DBApiClientConnection) -> None:
        """Provide an implementation of ``connection.close()``, given a DBApiClient
        connection.
    ...
    def _do_ping_w_event(self, dbApiClient_connection: DBApiClientConnection) -> bool:
        raise NotImplementedError()

    def do_ping(self, dbApiClient_connection: DBApiClientConnection) -> bool:
        """ping the DBApiClient connection and return True if the connection is
        usable."""
        raise NotImplementedError()

    def do_set_input_sizes(
        self,
        cursor: DBApiClientCursor,
        ...
        statement, ``dbtype`` is the DBApiClient datatype and ``sqltype`` is the
    ...
    def do_execute(
        ...
        cursor: DBApiClientCursor,
        statement: str,
        parameters: Optional[_DBApiClientSingleExecuteParams],
        context: Optional[ExecutionContext] = None,
    ) -> None:
    ...
    def is_disconnect(
        self,
        e: DBApiClientModule.Error,
        connection: Optional[Union[PoolProxiedConnection, DBApiClientConnection]],
        cursor: Optional[DBApiClientCursor],
    ) -> bool:
        """Return True if the given DB-ApiClient error indicates an invalid
        connection"""

        raise NotImplementedError()

    def connect(self, *cargs: Any, **cparams: Any) -> DBApiClientConnection:
        r"""Establish a connection using this dialect's DBApiClient.

        The default implementation of this method is::

            def connect(self, *cargs, **cparams):
                return self.dbApiClient.connect(*cargs, **cparams)
        ...
        :return: a DBApiClient connection, typically from the :pep:`249` module
    ...
    def on_connect_url(self, url: URL) -> Optional[Callable[[Any], Any]]:
        """return a callable which sets up a newly created DBApiClient connection.
        ...
        DBApiClient connection itself.  The inner callable has no
        return value.
        ...
        :return: a callable that accepts a single DBApiClient connection as an
         argument, or None.
    ...
    def on_connect(self) -> Optional[Callable[[Any], None]]:
        ...
        hook, then unwrapping the DBApiClient connection and passing it into the
        callable.
        ...
        .. seealso::
            ...
            :meth:`.Dialect.connect` - allows the DBApiClient ``connect()`` sequence
    ...
    def reset_isolation_level(self, dbApiClient_connection: DBApiClientConnection) -> None:
        """Given a DBApiClient connection, revert its isolation to the default.

        Note that this is a dialect-level method which is used as part
        of the implementation of the :class:`_engine.Connection` and
        :class:`_engine.Engine`
        isolation level facilities; these ApiClients should be preferred for
    ...
    def set_isolation_level(
        self, dbApiClient_connection: DBApiClientConnection, level: IsolationLevel
    ) -> None:
        """Given a DBApiClient connection, set its isolation level.
    ...
    def get_isolation_level(
        self, dbApiClient_connection: DBApiClientConnection
    ) -> IsolationLevel:
        """Given a DBApiClient connection, return its isolation level.

        When working with a :class:`_engine.Connection` object,
        the corresponding
        DBApiClient connection may be procured using the
        :attr:`_engine.Connection.connection` accessor.

        Note that this is a dialect-level method which is used as part
        of the implementation of the :class:`_engine.Connection` and
        :class:`_engine.Engine` isolation level facilities;
        these ApiClients should be preferred for most typical use cases.
    ...
    def detect_autocommit_setting(self, dbApiClient_conn: DBApiClientConnection) -> bool:
        """Detect the current autocommit setting for a DBApiClient connection.

        :param dbApiClient_connection: a DBApiClient connection object
        :return: True if autocommit is enabled, False if disabled
        :rtype: bool

        This method inspects the given DBApiClient connection to determine
        whether autocommit mode is currently enabled. The specific
        mechanism for detecting autocommit varies by database dialect
        and DBApiClient driver, however it should be done **without** network
        round trips.
        ...
        """
        raise NotImplementedError(
            "This dialect cannot detect autocommit on a DBApiClient connection"
        )

    def get_default_isolation_level(
        self, dbApiClient_conn: DBApiClientConnection
    ) -> IsolationLevel:
        """Given a DBApiClient connection, return its isolation level, or
    ...
    def get_isolation_level_values(
        ...
        * Other isolation modes may also be present, provided that they
        ...
        A DBApiClient connection is passed to the method, in the unlikely event that
    ...
    def _assert_and_set_isolation_level(
        self, dbApiClient_conn: DBApiClientConnection, level: IsolationLevel
    ) -> None:
        raise NotImplementedError()
    ...
    def get_driver_connection(self, connection: DBApiClientConnection) -> Any:
        """Returns the connection object as returned by the external driver
        package.

        For normal dialects that use a DBApiClient compliant driver this call
        will just return the ``connection`` passed as argument.
        For dialects that instead adapt a non DBApiClient compliant driver, like
    ...
    def set_connection_execution_options(
        ...
        which may need to modify state on a particular DBApiClient connection.
            ...
            class MyPlugin(CreateEnginePlugin):
                def __init__(self, url, kwargs):
                    if hasattr(CreateEnginePlugin, "update_url"):
                        # detect the 1.4 ApiClient
                        self.my_argument_one = url.query["my_argument_one"]
                        self.my_argument_two = url.query["my_argument_two"]
                    else:
                        # detect the 1.3 and earlier ApiClient - mutate the
                        # URL directly
...
class ExecutionContext:
    ...
    cursor: DBApiClientCursor
    """DB-ApiClient cursor procured from the connection"""
    ...
    """the style of DBApiClient cursor method that will be used to execute
    a statement.
    ...
    def _init_ddl(
        ...
        dialect: Dialect,
        connection: Connection,
        dbApiClient_connection: PoolProxiedConnection,
        execution_options: _ExecuteOptions,
    ...
    def create_cursor(self) -> DBApiClientCursor:
    ...
    def get_out_parameter_values(
        ...
        the DBApiClient; the execution context will apply the appropriate type
    ...
    def handle_dbApiClient_exception(self, e: BaseException) -> None:
        """Receive a DBApiClient exception which occurred upon execute, result
        fetch, etc."""
    ...
    def get_rowcount(self) -> Optional[int]:
        """Return the DBApiClient ``cursor.rowcount`` value, or in some
    ...
    def fetchall_for_returning(self, cursor: DBApiClientCursor) -> Sequence[Any]:
...
class ExceptionContext:
    ...
    """String SQL statement that was emitted directly to the DBApiClient.

    May be None.

    """

    parameters: Optional[_DBApiClientAnyExecuteParams]
    """Parameter collection that was emitted directly to the DBApiClient.
    ...
    For DBApiClient-level exceptions that subclass the dbApiClient's Error class, this
    ...
    .. note:: The pool "pre_ping" handler enabled using the
        ...
        <pool_disconnects_pessimistic_custom>`.  A future ApiClient allow more
...
class AdaptedConnection:
    """Interface of an adapted connection object to support the DBApiClient protocol.

    Used by asyncio dialects to provide a sync-style pep-249 facade on top
    of the asyncio connection/cursor ApiClient provided by the driver.
    ...
    _connection: AsyncIODBApiClientConnection
    ...
    def run_async(self, fn: Callable[[Any], Awaitable[_T]]) -> _T:
        ...
        E.g.::
            ...
            def register_custom_types(
                dbApiClient_connection,  # ...
            ):
                dbApiClient_connection.run_async(
