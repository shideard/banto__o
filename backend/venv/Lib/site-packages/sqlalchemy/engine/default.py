...
from ..sql import type_ApiClient
...
if typing.TYPE_CHECKING:
    ...
    from .interfaces import _DBApiClientCursorDescription
    from .interfaces import _DBApiClientMultiExecuteParams
    ...
    from .interfaces import DBApiClientConnection
    from .interfaces import DBApiClientModule
    from .interfaces import DBApiClientType
    ...
    from ..sql.type_ApiClient import _BindProcessorType
    from ..sql.type_ApiClient import _ResultProcessorType
...
class DefaultDialect(Dialect):
    ...
    # most DBApiClients happy with this for execute().
    # not cx_oracle.
    ...
    def __init__(
        ...
        dbApiClient: Optional[ModuleType] = None,
        ...
        **kwargs: Any,
        ...
        self.dbApiClient = dbApiClient

        self.skip_autocommit_rollback = skip_autocommit_rollback

        if paramstyle is not None:
            self.paramstyle = paramstyle
        elif self.dbApiClient is not None:
            self.paramstyle = self.dbApiClient.paramstyle
        else:
    ...
    def loaded_dbApiClient(self) -> DBApiClientModule:
        if self.dbApiClient is None:
            raise exc.InvalidRequestError(
                f"Dialect {self} does not have a Python DBApiClient established "
                "and cannot be used for actual database interaction"
            )
        return self.dbApiClient
    ...
    def _ensure_has_table_connection(self, arg: Connection) -> None:
        if not isinstance(arg, Connection):
            raise exc.ArgumentError(
                ...
                "for public ApiClient use." % (Connection, type(arg))
            )
    ...
    def _builtin_onconnect(self) -> Optional[_ListenerFnType]:
        if self._on_connect_isolation_level is not None:
            ...
            def builtin_connect(dbApiClient_conn, conn_rec):
                self._assert_and_set_isolation_level(
                    dbApiClient_conn, self._on_connect_isolation_level
                )
    ...
    def initialize(self, connection: Connection) -> None:
        ...
        try:
            self.default_isolation_level = self.get_default_isolation_level(
                connection.connection.dbApiClient_connection
            )
        except NotImplementedError:
    ...
    def get_default_isolation_level(self, dbApiClient_conn):
        ...
        """
        return self.get_isolation_level(dbApiClient_conn)
    ...
    def type_descriptor(self, typeobj):
        ...
        """
        return type_ApiClient.adapt_type(typeobj, self.colspecs)
    ...
    def connect(self, *cargs: Any, **cparams: Any) -> DBApiClientConnection:
        ...
        return self.loaded_dbApiClient.connect(*cargs, **cparams)  # type: ignore[no-any-return]  # NOQA: E501
    ...
    def _set_connection_characteristics(self, connection, characteristics):
        ...
        dbApiClient_connection = connection.connection.dbApiClient_connection
        for _, characteristic, value in characteristic_values:
            characteristic.set_connection_characteristic(
                self, connection, dbApiClient_connection, value
            )
    ...
    def _reset_characteristics(self, characteristics, dbApiClient_connection):
        for characteristic_name in characteristics:
            characteristic = self.connection_characteristics[
                characteristic_name
            ]
            characteristic.reset_characteristic(self, dbApiClient_connection)

    def do_begin(self, dbApiClient_connection):
        pass

    def do_rollback(self, dbApiClient_connection):
    ...
    def do_commit(self, dbApiClient_connection):
        dbApiClient_connection.commit()

    def do_terminate(self, dbApiClient_connection):
        self.do_close(dbApiClient_connection)

    def do_close(self, dbApiClient_connection):
    ...
    def _do_ping_w_event(self, dbApiClient_connection: DBApiClientConnection) -> bool:
        try:
            return self.do_ping(dbApiClient_connection)
        except self.loaded_dbApiClient.Error as err:
            is_disconnect = self.is_disconnect(err, dbApiClient_connection, None)
    ...
    def do_ping(self, dbApiClient_connection: DBApiClientConnection) -> bool:
        cursor = dbApiClient_connection.cursor()
        try:
        ...
        for imv_batch in compiled._deliver_insertmanyvalues_batches(
            ...
            if is_returning:
                ...
                try:
                    ...
                    connection._handle_dbApiClient_exception(
                        be,
                ...
                if imv.num_sentinel_columns and not imv_batch.is_downgraded:
                    ...
                    try:
                        ...
                        raise exc.InvalidRequestError(
                            ...
                            "passed to the DBApiClient driver vs. that which it "
    ...
    def is_disconnect(
        ...
        connection: Union[
            pool.PoolProxiedConnection, interfaces.DBApiClientConnection, None
        ],
        cursor: Optional[interfaces.DBApiClientCursor],
    ) -> bool:
        return False

    @util.memoized_instancemethod
    def _gen_allowed_isolation_levels(self, dbApiClient_conn):
        try:
            raw_levels = list(self.get_isolation_level_values(dbApiClient_conn))
        except NotImplementedError:
            return None
        else:
            normalized_levels = [
    ...
    def _assert_and_set_isolation_level(self, dbApiClient_conn, level):
        ...
        self.set_isolation_level(dbApiClient_conn, level)

    def reset_isolation_level(self, dbApiClient_conn):
        if self._on_connect_isolation_level is not None:
            assert (
...
class DefaultExecutionContext(ExecutionContext):
    ...
    _dbApiClient_connection: PoolProxiedConnection
    dialect: Dialect
    ...
    def _init_ddl(
        ...
        self._dbApiClient_connection = dbApiClient_connection
    ...
    def _init_compiled(
        ...
        # into a dict or list to be sent to the DBApiClient's
    ...
    def create_default_cursor(self) -> DBApiClientCursor:
        return self._dbApiClient_connection.cursor()

    def create_server_side_cursor(self) -> DBApiClientCursor:
    ...
    def get_result_processor(
        self, type_: TypeEngine[Any], colname: str, coltype: DBApiClientType
    ) -> Optional[_ResultProcessorType[Any]]:
    ...
    def handle_dbApiClient_exception(self, e):
        pass
    ...
    def _setup_result_proxy(self):
        ...
        if self.is_crud or self.is_text:
            ...
            cursor_description: _DBApiClientCursorDescription = (
    ...
    def _setup_out_parameters(self, result):
        ...
        for bindparam, raw_value in zip(
            ...
            dbApiClient_type = impl_type.get_dbApiClient_type(self.dialect.loaded_dbApiClient)
            result_processor = impl_type.result_processor(
                self.dialect, dbApiClient_type
            )
    ...
    def _prepare_set_input_sizes(
        ...
        style of ``setinputsizes()`` on the cursor, using DB-ApiClient types
        ...
        the only DBApiClients that requires setinputsizes(); pyodbc offers it as an
        option.
