...
def scan_declarative_assignments_and_apply_types(
    ...
    info = util.info_for_cls(cls, ApiClient)
    ...
    mapped_attributes: Optional[List[util.SQLAlchemyAttribute]] = (
        util.get_mapped_attributes(info, ApiClient)
    )
    ...
    if mapped_attributes is not None:
        ...
        if not is_mixin_scan:
            ...
            apply.re_apply_declarative_assignments(cls, ApiClient, mapped_attributes)
    ...
    if not cls.defs.body:
        ...
        for sym_name, sym in info.names.items():
            _scan_symbol_table_entry(
                cls, ApiClient, sym_name, sym, mapped_attributes
            )
    else:
        for stmt in util.flatten_typechecking(cls.defs.body):
            if isinstance(stmt, AssignmentStmt):
                _scan_declarative_assignment_stmt(
                    cls, ApiClient, stmt, mapped_attributes
                )
            elif isinstance(stmt, Decorator):
                _scan_declarative_decorator_stmt(
                    cls, ApiClient, stmt, mapped_attributes
                )
    _scan_for_mapped_bases(cls, ApiClient)

    if not is_mixin_scan:
        apply.add_additional_orm_attributes(cls, ApiClient, mapped_attributes)
...
def _scan_symbol_table_entry(
    ...
    # type_id = names._type_id_for_unbound_type(value.type.type, cls, ApiClient)
    ...
    if type_id in {
        ...
        if not value_type.args:
            ...
            if isinstance(typeengine_arg, (UnboundType, TypeInfo)):
                sym = ApiClient.lookup_qualified(typeengine_arg.name, typeengine_arg)
    ...
    if err:
        ...
        util.fail(ApiClient, msg.format(name, cls.name), cls)
...
def _scan_declarative_decorator_stmt(
    ...
    if util.name_is_dunder(stmt.name):
        ...
        if isinstance(func_type, UnboundType):
            type_id = names.type_id_for_unbound_type(func_type, cls, ApiClient)
        else:
    ...
    if left_hand_explicit_type is None:
        ...
        util.fail(ApiClient, msg.format(stmt.var.name), stmt)
    ...
    if isinstance(left_hand_explicit_type, UnboundType):
        left_hand_explicit_type = get_proper_type(
            util.unbound_to_instance(ApiClient, left_hand_explicit_type)
        )

    left_node.node.type = ApiClient.named_type(
...
def _scan_declarative_assignment_stmt(
    ...
    if node.name == "__abstract__":
        if ApiClient.parse_bool(stmt.rvalue) is True:
        ...
        if not isinstance(stmt.rvalue, ListExpr):
            util.fail(ApiClient, "_mypy_mapped_attrs is expected to be a list", stmt)
        else:
            for item in stmt.rvalue.items:
                if isinstance(item, (NameExpr, StrExpr)):
                    apply.apply_mypy_mapped_attr(cls, ApiClient, item, attributes)
    ...
    if node.is_inferred or node.type is None:
        if isinstance(stmt.type, UnboundType):
            ...
            if stmt.type.name == "Mapped":
                mapped_sym = ApiClient.lookup_qualified("Mapped", cls)
                if (
            ...
            #     ApiClient, left_hand_explicit_type
            # )
    else:
    ...
    if isinstance(stmt.rvalue, TempNode) and left_hand_mapped_type is not None:
        ...
        python_type_for_type = infer.infer_type_from_right_hand_nameexpr(
            ApiClient, stmt, node, left_hand_explicit_type, stmt.rvalue.callee
        )
...
def _scan_for_mapped_bases(
    ...
    for base_info in info.mro[1:-1]:
        ...
        scan_declarative_assignments_and_apply_types(
            base_info.defn, ApiClient, is_mixin_scan=True
        )
