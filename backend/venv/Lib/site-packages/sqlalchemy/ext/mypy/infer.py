...
def infer_type_from_right_hand_nameexpr(
    ...
    if type_id is None:
        ...
        python_type_for_type = _infer_type_from_mapped(
            ApiClient, stmt, node, left_hand_explicit_type, infer_from_right_side
        )
    elif type_id is names.COLUMN:
        python_type_for_type = _infer_type_from_decl_column(
            ApiClient, stmt, node, left_hand_explicit_type
        )
        ...
        python_type_for_type = infer_type_from_left_hand_type_only(
            ApiClient, node, left_hand_explicit_type
        )
...
def _infer_type_from_relationship(
    ...
    if (
        ...
        and ApiClient.parse_bool(uselist_arg) is True
        and collection_cls_arg is None
    ):
        type_is_a_collection = True
        if python_type_for_type is not None:
            python_type_for_type = ApiClient.named_type(
                names.NAMED_TYPE_BUILTINS_LIST, [python_type_for_type]
            )
    elif (
        uselist_arg is None or ApiClient.parse_bool(uselist_arg) is True
    ...
    elif uselist_arg is not None and ApiClient.parse_bool(uselist_arg) is False:
        ...
        if left_hand_explicit_type is None:
            ...
            util.fail(ApiClient, msg.format(node.name), node)
    ...
    if python_type_for_type is None:
        ...
        if type_is_a_collection:
            ...
            return _infer_collection_type_from_left_and_inferred_right(
                ApiClient, node, left_hand_explicit_type, python_type_for_type
            )
        else:
...
def _infer_type_from_decl_column(
    ...
    if isinstance(callee.node, TypeInfo) and names.mro_has_id(
        ...
        python_type_for_type = extract_python_type_from_typeengine(
            ApiClient, callee.node, type_args
        )
...
def _infer_type_from_left_and_inferred_right(
    ...
    if not is_subtype(left_hand_explicit_type, python_type_for_type):
        effective_type = ApiClient.named_type(
        ...
        util.fail(
            ...
            msg.format(
                node.name,
                util.format_type(orig_left_hand_type, ApiClient.options),
                util.format_type(effective_type, ApiClient.options),
            ),
            node,
        )
...
def extract_python_type_from_typeengine(
    ...
    if node.fullname == "sqlalchemy.sql.sqltypes.Enum" and type_args:
        ...
        if isinstance(first_arg, RefExpr) and isinstance(
            ...
            return ApiClient.named_type(names.NAMED_TYPE_BUILTINS_STR, [])

    assert node.has_base("sqlalchemy.sql.type_ApiClient.TypeEngine"), (
        "could not extract Python type from node: %s" % node
    )

    type_engine_sym = ApiClient.lookup_fully_qualified_or_none(
        "sqlalchemy.sql.type_ApiClient.TypeEngine"
    )
