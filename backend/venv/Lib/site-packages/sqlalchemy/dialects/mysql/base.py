...
connection.  For the special AUTOCOMMIT isolation level, DBApiClient-specific
techniques are used.
...
attributes provided by specific DBApiClients, and is currently supported by
...
Most MySQL / MariaDB DBApiClients offer the option to set the client character set for
...
MySQL DBApiClients will default this to a value such as ``latin1``, and some
will make use of the ``default-character-set`` setting in the ``my.cnf``
file as well.   Documentation for the DBApiClient in use should be consulted
...
All modern DBApiClients should support the ``utf8mb4`` charset.
...
These character set introducers are provided by the DBApiClient driver, assuming the
...
the ``first_connect`` and ``connect`` events::
    ...
    def connect(dbApiClient_connection, connection_record):
...
statement on the connection at the moment a particular DBApiClient connection is
...
SQLAlchemy standardizes the DBApiClient ``cursor.rowcount`` attribute to be the
usual definition of "number of rows matched by an UPDATE or DELETE" statement.
This is in contradiction to the default setting on most MySQL DBApiClient drivers,
...
class MySQLDialect(default.DefaultDialect):
    ...
    # all other MySQL DBApiClients require a conversion routine
    ...
    def _is_mariadb_from_url(cls, url: URL) -> bool:
        dbApiClient = cls.import_dbApiClient()
        dialect = cls(dbApiClient=dbApiClient)
    ...
    def _get_server_version_info(
        ...
        dbApiClient_con = connection.connection
        cursor = dbApiClient_con.cursor()
    ...
    def is_disconnect(
        ...
        if isinstance(
            e,
            (
                self.dbApiClient.OperationalError,  # type: ignore
                self.dbApiClient.ProgrammingError,  # type: ignore
                self.dbApiClient.InterfaceError,  # type: ignore
            ),
            ...
            e, (self.dbApiClient.InterfaceError, self.dbApiClient.InternalError)  # type: ignore  # noqa: E501
        ):
    ...
    def _extract_error_code(
        self, exception: DBApiClientModule.Error
    ) -> Optional[int]:
        raise NotImplementedError()
    ...
    def has_table(
        ...
        **kw: Any,
        ...
        except exc.DBApiClientError as e:
