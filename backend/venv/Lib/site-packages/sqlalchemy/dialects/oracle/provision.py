...
def _connect_with_retry(dialect, conn_rec, cargs, cparams):
    ...
    for _ in range(5):
        try:
            return dialect.loaded_dbApiClient.connect(*cargs, **cparams)
        except (
            dialect.loaded_dbApiClient.DatabaseError,
            dialect.loaded_dbApiClient.OperationalError,
        ) as err:
            err_ = err
...
def _oracle_post_configure_engine(url, engine, follower_ident):
    ...
    def checkin(dbApiClient_connection, connection_record):
        ...
        try:
            sc = dbApiClient_connection.stmtcachesize
        except:
            # connection closed
            pass
        else:
            dbApiClient_connection.stmtcachesize = 0
            dbApiClient_connection.stmtcachesize = sc
...
def _oracle_set_default_schema_on_connection(
    cfg, dbApiClient_connection, schema_name
):
