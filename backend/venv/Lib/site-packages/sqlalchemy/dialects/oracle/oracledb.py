...
<https://python-oracledb.readthedocs.io/en/latest/ApiClient_manual/module.html#oracledb.init_oracle_client>`_
...
<https://python-oracledb.readthedocs.io/en/latest/ApiClient_manual/module.html#oracledb.connect>`_
...
<https://python-oracledb.readthedocs.io/en/latest/ApiClient_manual/module.html#oracledb.create_pool>`_
...
The python-oracle DBApiClient has a deep and fundamental reliance upon the usage of
the DBApiClient ``setinputsizes()`` call.  The purpose of this call is to establish
the datatypes that are bound to a SQL statement for Python values being passed
as parameters.  While virtually no other DBApiClient assigns any use to the
``setinputsizes()`` call, the python-oracledb DBApiClient relies upon it heavily in
...
<https://python-oracledb.readthedocs.io/en/latest/ApiClient_manual/module.html#database-types>`_
...
requires python-oracledb 2.3).  ApiClients for two phase transactions are provided at
...
underlying python-oracledb DBApiClient for all those cases where the driver can make
the best decision.
...
from ... import exc
from ... import pool
from ...connectors.asyncio import AsyncAdapt_dbApiClient_connection
from ...connectors.asyncio import AsyncAdapt_dbApiClient_cursor
from ...connectors.asyncio import AsyncAdapt_dbApiClient_ss_cursor
from ...connectors.asyncio import AsyncAdaptFallback_dbApiClient_connection
...
class OracleDialect_oracledb(_cx_oracle.OracleDialect_cx_oracle):
    ...
    def is_thin_mode(cls, connection):
        return connection.connection.dbApiClient_connection.thin
...
class AsyncAdapt_oracledb_cursor(AsyncAdapt_dbApiClient_cursor):
    _cursor: AsyncCursor
...
class AsyncAdapt_oracledb_ss_cursor(
    AsyncAdapt_dbApiClient_ss_cursor, AsyncAdapt_oracledb_cursor
):
    __slots__ = ()
...
class AsyncAdapt_oracledb_connection(AsyncAdapt_dbApiClient_connection):
...
class AsyncAdaptFallback_oracledb_connection(
    AsyncAdaptFallback_dbApiClient_connection, AsyncAdapt_oracledb_connection
):
    __slots__ = ()


class OracledbAdaptDBApiClient:
...
class OracleExecutionContextAsync_oracledb(OracleExecutionContext_oracledb):
    ...
    def create_server_side_cursor(self):
        c = self._dbApiClient_connection.ss_cursor()
        if self.dialect.arraysize:
            c.arraysize = self.dialect.arraysize
...
class OracleDialectAsync_oracledb(OracleDialect_oracledb):
    ...
    def import_dbApiClient(cls):
        ...
        return OracledbAdaptDBApiClient(oracledb)
