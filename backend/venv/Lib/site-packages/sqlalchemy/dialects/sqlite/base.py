...
on top of it) has several quirks, workarounds, and ApiClient features in the
...
* **Enabling modern sqlite3 transaction control via the autocommit connect parameter** (Python 3.12 and above)
  ...
  attribute on its ``Connection`` object::
    ...
    def do_connect(dbApiClient_connection, connection_record):
...
autocommit support that is introduced in the section :ref:`dbApiClient_autocommit`.
...
new connections through the usage of events::
    ...
    @event.listens_for(Engine, "connect")
    def set_sqlite_pragma(dbApiClient_connection, connection_record):
        # the sqlite3 driver will not set PRAGMA foreign_keys
        # if autocommit=False; set to True temporarily
        ac = dbApiClient_connection.autocommit
        ...
        dbApiClient_connection.autocommit = ac
...
.. seealso::
    ...
    :ref:`event_toplevel` - SQLAlchemy event ApiClient.
...
:meth:`_engine.CursorResult.keys` and :meth:`.Row.keys()` in the public ApiClient. In
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.dbApiClient is not None:
            if self.dbApiClient.sqlite_version_info < (3, 7, 16):
                util.warn(
                    ...
                    % (self.dbApiClient.sqlite_version_info,)
                )

            # NOTE: python 3.7 on fedora for me has SQLite 3.34.1.  These
            # version checks are getting very stale.
            self._broken_dotted_colnames = self.dbApiClient.sqlite_version_info < (
                3,
                10,
                0,
            )
            self.supports_default_values = self.dbApiClient.sqlite_version_info >= (
                3,
                3,
                8,
            )
            self.supports_cast = self.dbApiClient.sqlite_version_info >= (3, 2, 3)
            ...
            self._broken_fk_pragma_quotes = self.dbApiClient.sqlite_version_info < (
                3,
                6,
                14,
            )

            if self.dbApiClient.sqlite_version_info < (3, 35) or util.pypy:
                self.update_returning = self.delete_returning = (
                    self.insert_returning
                ) = False

            if self.dbApiClient.sqlite_version_info < (3, 32, 0):
