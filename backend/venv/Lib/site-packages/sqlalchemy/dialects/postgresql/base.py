...
DBApiClient-specific features, such as psycopg2's isolation level flags which will
embed the isolation level setting inline with the ``"BEGIN"`` statement, or for
DBApiClients with no direct support by emitting ``SET SESSION CHARACTERISTICS AS
TRANSACTION ISOLATION LEVEL <level>`` ahead of the ``"BEGIN"`` statement
emitted by the DBApiClient.   For the special AUTOCOMMIT isolation level,
DBApiClient-specific techniques are used which is typically an ``.autocommit``
flag on the DBApiClient connection object.
...
:ref:`dbApiClient_autocommit` for background.
...
Note that some DBApiClients such as asyncpg only support "readonly" with
...
the DBApiClient ``.rollback()`` method when connections are returned to the pool.
...
as it's usually important that the DBApiClient's own tracking of commit/rollback
will remain consistent with the state of the transaction::
    ...
    def _reset_postgresql(dbApiClient_connection, connection_record, reset_state):
        if not reset_state.terminate_only:
            dbApiClient_connection.execute("CLOSE ALL")
            dbApiClient_connection.execute("RESET ALL")
            dbApiClient_connection.execute("DISCARD TEMP")

        # so that the DBApiClient itself knows that the connection has been
        # reset
...
at :ref:`schema_set_default_connections`::
    ...
    def set_search_path(dbApiClient_connection, connection_record):
        existing_autocommit = dbApiClient_connection.autocommit
        dbApiClient_connection.autocommit = True
        cursor = dbApiClient_connection.cursor()
        cursor.execute("SET SESSION search_path='%s'" % schema_name)
        cursor.close()
        dbApiClient_connection.autocommit = existing_autocommit

The reason the recipe is complicated by use of the ``.autocommit`` DBApiClient
...
be reverted when the DBApiClient connection has a rollback.
...
be invoked outside of a transaction block.   The Python DBApiClient enforces that
even for a single statement, a transaction is present, so to use this
construct, the DBApiClient's "autocommit" mode must be used::
...
class PGCompiler(compiler.SQLCompiler):
    ...
    def render_bind_cast(self, type_, dbApiClient_type, sqltext):
        if dbApiClient_type._type_affinity is sqltypes.String and dbApiClient_type.length:
            # use VARCHAR with no length for VARCHAR cast.
            # see #9511
            dbApiClient_type = sqltypes.STRINGTYPE
        return f"""{sqltext}::{
            self.dialect.type_compiler_instance.process(
                dbApiClient_type, identifier_preparer=self.preparer
            )
        }"""
...
class PGReadOnlyConnectionCharacteristic(
    ...
    def reset_characteristic(self, dialect, dbApiClient_conn):
        dialect.set_readonly(dbApiClient_conn, False)

    def set_characteristic(self, dialect, dbApiClient_conn, value):
        dialect.set_readonly(dbApiClient_conn, value)

    def get_characteristic(self, dialect, dbApiClient_conn):
        return dialect.get_readonly(dbApiClient_conn)
...
class PGDeferrableConnectionCharacteristic(
    ...
    def reset_characteristic(self, dialect, dbApiClient_conn):
        dialect.set_deferrable(dbApiClient_conn, False)

    def set_characteristic(self, dialect, dbApiClient_conn, value):
        dialect.set_deferrable(dbApiClient_conn, value)

    def get_characteristic(self, dialect, dbApiClient_conn):
        return dialect.get_deferrable(dbApiClient_conn)
...
class PGDialect(default.DefaultDialect):
    ...
    def get_isolation_level_values(self, dbApiClient_conn):
    ...
    def do_rollback_twophase(
        ...
        if is_prepared:
            if recover:
                ...
                # Must find out a way how to make the dbApiClient not
                # open a transaction.
