...
statement invocation.   The cache is on a per-DBApiClient connection basis, which
means that the primary storage for prepared statements is within DBApiClient
connections pooled within the connection pool.   The size of this cache
defaults to 100 statements per DBApiClient connection and may be adjusted using the
``prepared_statement_cache_size`` DBApiClient argument (note that while this argument
is implemented by SQLAlchemy, it is part of the DBApiClient emulation portion of the
asyncpg dialect, therefore is handled as a DBApiClient argument, not a dialect
argument)::
...
class _AsyncpgRange(ranges.AbstractSingleRangeImpl):
    def bind_processor(self, dialect):
        asyncpg_Range = dialect.dbApiClient.asyncpg.Range
...
class PGExecutionContext_asyncpg(PGExecutionContext):
    def handle_dbApiClient_exception(self, e):
        if isinstance(
            e,
            (
                self.dialect.dbApiClient.InvalidCachedStatementError,
                self.dialect.dbApiClient.InternalServerError,
            ),
        ):
...
class AsyncAdapt_asyncpg_connection(AsyncAdapt_terminate, AdaptedConnection):
    ...
    def _handle_exception(self, error):
        ...
        if not isinstance(error, AsyncAdapt_asyncpg_dbApiClient.Error):
            exception_mapping = self.dbApiClient._asyncpg_error_translate
    ...
    def _terminate_handled_exceptions(self):
        return super()._terminate_handled_exceptions() + (
            self.dbApiClient.asyncpg.PostgresError,
        )
...
class AsyncAdapt_asyncpg_dbApiClient:
...
class PGDialect_asyncpg(PGDialect):
    ...
    @util.memoized_property
    def _dbApiClient_version(self):
        if self.dbApiClient and hasattr(self.dbApiClient, "__version__"):
            return tuple(
                [
                    int(x)
                    for x in re.findall(
                        r"(\d+)(?:[-\.]?|$)", self.dbApiClient.__version__
                    )
                ]
            )
        else:
    ...
    def import_dbApiClient(cls):
        return AsyncAdapt_asyncpg_dbApiClient(__import__("asyncpg"))
    ...
    def do_terminate(self, dbApiClient_connection) -> None:
    ...
    def do_ping(self, dbApiClient_connection):
        dbApiClient_connection.ping()
        return True
    ...
    def is_disconnect(self, e, connection, cursor):
        if connection:
            ...
            return isinstance(
                e, self.dbApiClient.InterfaceError
