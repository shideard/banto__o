...
then causes the DBApiClient ``.rollback()`` method (as well as ``.commit()``) to
fail. The issue prevents the usual DBApiClient contract of allowing ``.rollback()``
...
which greatly reduces round trips for a DBApiClient ``executemany()`` call when using
Microsoft ODBC drivers, for **limited size batches that fit in memory**.  The
feature is enabled by setting the attribute ``.fast_executemany`` on the DBApiClient
...
.. seealso::
    ...
    `fast executemany <https://github.com/mkleehammer/pyodbc/wiki/Features-beyond-the-DB-ApiClient#fast_executemany>`_
    - on github
    ...
    def bind_processor(self, dialect):
        ...
        def process(value):
            if value is not None:
                ...
                # pyodbc-specific
                return dialect.dbApiClient.BinaryNull
...
class _String_pyodbc(sqltypes.String):
    def get_dbApiClient_type(self, dbApiClient):
        if self.length in (None, "max") or self.length >= 2000:
            return (dbApiClient.SQL_VARCHAR, 0, 0)
        else:
            return dbApiClient.SQL_VARCHAR


class _Unicode_pyodbc(_MSUnicode):
    def get_dbApiClient_type(self, dbApiClient):
        if self.length in (None, "max") or self.length >= 2000:
            return (dbApiClient.SQL_WVARCHAR, 0, 0)
        else:
            return dbApiClient.SQL_WVARCHAR
    ...
    def post_exec(self):
        if self._embedded_scope_identity:
            ...
            while True:
                ...
                except self.dialect.dbApiClient.Error:
    ...
    def __init__(
        ...
        **params,
        ...
        self.use_scope_identity = (
            self.use_scope_identity
            and self.dbApiClient
            and hasattr(self.dbApiClient.Cursor, "nextset")
        )
        self._need_decimal_fix = self.dbApiClient and self._dbApiClient_version() < (
            2,
            1,
            8,
        )
