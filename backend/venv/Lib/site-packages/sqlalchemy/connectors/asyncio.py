...
"""generic asyncio-adapted versions of DBApiClient connection and cursor"""
...
if TYPE_CHECKING:
    from ..engine.interfaces import _DBApiClientCursorDescription
    from ..engine.interfaces import _DBApiClientMultiExecuteParams
    from ..engine.interfaces import _DBApiClientSingleExecuteParams
    from ..engine.interfaces import DBApiClientModule
    from ..util.typing import Self


class AsyncIODBApiClientConnection(Protocol):
    ...
    # note that async DBApiClients dont agree if close() should be awaitable,
    # so it is omitted here and picked up by the __getattr__ hook below

    async def commit(self) -> None: ...

    def cursor(self, *args: Any, **kwargs: Any) -> AsyncIODBApiClientCursor: ...
...
class AsyncIODBApiClientCursor(Protocol):
...
class AsyncAdapt_dbApiClient_module:
    if TYPE_CHECKING:
        Error = DBApiClientModule.Error
        OperationalError = DBApiClientModule.OperationalError
        InterfaceError = DBApiClientModule.InterfaceError
        IntegrityError = DBApiClientModule.IntegrityError

        def __getattr__(self, key: str) -> Any: ...


class AsyncAdapt_dbApiClient_cursor:
    server_side = False
    __slots__ = (
        "_adapt_connection",
        "_connection",
    ...
    _cursor: AsyncIODBApiClientCursor
    _adapt_connection: AsyncAdapt_dbApiClient_connection
    _connection: AsyncIODBApiClientConnection
    _rows: Deque[Any]

    def __init__(self, adapt_connection: AsyncAdapt_dbApiClient_connection):
    ...
    def _aenter_cursor(self, cursor: AsyncIODBApiClientCursor) -> AsyncIODBApiClientCursor:
        return self.await_(cursor.__aenter__())  # type: ignore[no-any-return]

    def _make_new_cursor(
        self, connection: AsyncIODBApiClientConnection
    ) -> AsyncIODBApiClientCursor:
        return connection.cursor()

    @property
    def description(self) -> Optional[_DBApiClientCursorDescription]:
    ...
    def executemany(
        self,
        operation: Any,
        seq_of_parameters: _DBApiClientMultiExecuteParams,
    ) -> Any:
        try:
            return self.await_(
    ...
    async def _execute_async(
        self, operation: Any, parameters: Optional[_DBApiClientSingleExecuteParams]
    ) -> Any:
...
class AsyncAdapt_dbApiClient_ss_cursor(AsyncAdapt_dbApiClient_cursor):
    __slots__ = ()
    server_side = True
...
class AsyncAdapt_dbApiClient_connection(AdaptedConnection):
    _cursor_cls = AsyncAdapt_dbApiClient_cursor
    _ss_cursor_cls = AsyncAdapt_dbApiClient_ss_cursor

    await_ = staticmethod(await_only)

    __slots__ = ("dbApiClient", "_execute_mutex")

    _connection: AsyncIODBApiClientConnection

    def __init__(self, dbApiClient: Any, connection: AsyncIODBApiClientConnection):
        self.dbApiClient = dbApiClient
        self._connection = connection
        self._execute_mutex = asyncio.Lock()

    def cursor(self, server_side: bool = False) -> AsyncAdapt_dbApiClient_cursor:
        if server_side:
    ...
    def execute(
        ...
        """lots of DBApiClients seem to provide this, so include it"""
        cursor = self.cursor()
...
class AsyncAdaptFallback_dbApiClient_connection(AsyncAdapt_dbApiClient_connection):
    __slots__ = ()
...
class AsyncAdapt_terminate:
    """Mixin for a AsyncAdapt_dbApiClient_connection to add terminate support."""
