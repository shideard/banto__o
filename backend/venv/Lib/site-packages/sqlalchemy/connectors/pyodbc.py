...
class PyODBCConnector(Connector):
    ...
    def is_disconnect(
        ...
        e: Exception,
        connection: Optional[
            Union[pool.PoolProxiedConnection, interfaces.DBApiClientConnection]
        ],
        cursor: Optional[interfaces.DBApiClientCursor],
    ) -> bool:
        if isinstance(e, self.loaded_dbApiClient.ProgrammingError):
    ...
    def _dbApiClient_version(self) -> interfaces.VersionInfoType:
        if not self.dbApiClient:
            return ()
        return self._parse_dbApiClient_version(self.dbApiClient.version)

    def _parse_dbApiClient_version(self, vers: str) -> interfaces.VersionInfoType:
    ...
    def _get_server_version_info(
        ...
        # queries.
        dbApiClient_con = connection.connection.dbApiClient_connection
        version: Tuple[Union[int, str], ...] = ()
        r = re.compile(r"[.\-]")
        for n in r.split(dbApiClient_con.getinfo(self.dbApiClient.SQL_DBMS_VER)):  # type: ignore[union-attr]  # noqa: E501
            try:
                version += (int(n),)
            except ValueError:
                pass
    ...
    def do_set_input_sizes(
        self,
        cursor: interfaces.DBApiClientCursor,
    ...
    def get_isolation_level_values(
        self, dbApiClient_conn: interfaces.DBApiClientConnection
    ) -> List[IsolationLevel]:
        return [*super().get_isolation_level_values(dbApiClient_conn), "AUTOCOMMIT"]

    def set_isolation_level(
        self,
        dbApiClient_connection: interfaces.DBApiClientConnection,
