...
if TYPE_CHECKING:
    ...
    from ..orm.decl_ApiClient import DeclarativeAttributeIntercept
    from ..orm.decl_ApiClient import DeclarativeMeta
...
class SessionEvents(event.Events[Session]):
    ...
    def do_orm_execute(self, orm_execute_state: ORMExecuteState) -> None:
        ...
        .. note::  The :meth:`_orm.SessionEvents.do_orm_execute` event hook
           ...
           executions regardless of whether the Core or ORM ApiClients are in use,
    ...
    def after_rollback(self, session: Session) -> None:
        """Execute after a real DBApiClient rollback has occurred.

        Note that this event only fires when the *actual* rollback against
        the database occurs - it does *not* fire each time the
        :meth:`.Session.rollback` method is called, if the underlying
        DBApiClient transaction has already been rolled back.  In many
    ...
    def after_soft_rollback(
        ...
        rollbacks that don't actually emit at the DBApiClient level.

        This corresponds to both nested and outer rollbacks, i.e.
        the innermost rollback that calls the DBApiClient's
