...
class NullPool(Pool):
    ...
    Instead it literally opens and closes the underlying DB-ApiClient connection
...
class StaticPool(Pool):
    ...
    def dispose(self) -> None:
        if (
            ...
            and self.connection.dbApiClient_connection is not None
        ):
    ...
    def _transfer_from(self, other_static_pool: StaticPool) -> None:
        # used by the test suite to make a new engine / pool without
        # losing the state of an existing SQLite :memory: connection
        def creator(rec: ConnectionPoolEntry) -> DBApiClientConnection:
            conn = other_static_pool.connection.dbApiClient_connection
